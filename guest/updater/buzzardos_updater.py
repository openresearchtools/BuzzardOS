...
BUS_NAME = "org.openresearchtools.BuzzardOS.Updater1"
INTERFACE = BUS_NAME
OBJECT_PATH = "/org/openresearchtools/BuzzardOS/Updater1"
INTERACTIVE_UID = 1000
MAX_SENDER_UID_CACHE = 4_096

INTROSPECTION_XML = """
<node>
  <interface name="org.openresearchtools.BuzzardOS.Updater1">
    <method name="Check">
...
def _transient_worker_command(
    ...
    if not unit.startswith(f"buzzardos-update-{operation}-") or not all(
...
class SystemBusService:
    ...
    def _start_worker(self, operation: str, generation: str | None = None) -> int:
        with self.worker_lock:
            ...
            unit = f"buzzardos-update-{operation}-{uuid.uuid4().hex}"
