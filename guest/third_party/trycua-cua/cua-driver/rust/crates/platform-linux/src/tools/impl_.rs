...
async fn capture_guest_output() -> Option<Vec<u8>> {
    tokio::task::spawn_blocking(crate::capture::screenshot_display_bytes)
        .await
        .ok()
        .and_then(Result::ok)
}
...
async fn capture_guest_output_after_pointer_at(
    ...
    let previous_pointer = if crate::wayland::wayland_input_enabled() {
    ...
    let positioned = tokio::task::spawn_blocking(move || {
        if crate::wayland::wayland_input_enabled() {
            crate::wayland::move_cursor_absolute(None, x, y)
        } else {
            crate::input::send_move_xtest_desktop(x, y)
        }
    })
    .await
    .ok()
...
async fn capture_guest_output_with_current_pointer_mask() -> (Option<Vec<u8>>, Vec<(i32, i32)>) {
    let cursor_masks = if crate::wayland::wayland_input_enabled() {
        crate::wayland::last_synth_cursor_pos()
            .into_iter()
            .collect()
    } else {
...
mod list_apps_tests {
    ...
    fn installed(name: &str, launch_path: &str) -> crate::installed_apps::InstalledApp {
                    ...
                    if let Some(snapshot_id) = snapshot_id {
                        let snapshot_handle =
                            cua_driver_core::element_token::token_for(snapshot_id, 0)
                                .trim_end_matches(":0")
                                .to_string();
                        crate::atspi::register_snapshot_element_keys(
                            pid,
                            &snapshot_handle,
                            &tr.nodes,
                        );
...
fn mouse_hold_path(desktop: bool) -> &'static str {
    match (crate::wayland::is_wayland(), desktop) {
...
impl Tool for ClickTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if has_xy && !has_pid && !has_window_id {
            ...
            return match r {
                Ok(Ok(())) => {
                    let path = if crate::wayland::wayland_input_enabled() {
                        "wayland_desktop"
                    } else {
                        "xtest_desktop"
                    };
        ...
        if let Some(idx) = elem_idx_resolved {
            ...
            let stable_element_key = snapshot_handle
                .as_deref()
                .and_then(|snapshot| crate::atspi::snapshot_element_key(pid, snapshot, idx));
            ...
            if click_can_use_atspi_action(modifiers.is_empty(), button, count) {
                let ax_result = tokio::task::spawn_blocking(move || {
                    if let Some(key) = stable_element_key {
                        crate::atspi::perform_action_by_key(pid, key)
                    } else {
            ...
            let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                ...
                if crate::wayland::wayland_input_enabled() {
                    ...
                    return crate::wayland::click_desktop(
                        sx.round() as i32,
                        sy.round() as i32,
                        count as u32,
                        button,
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
            ...
            && !is_webkitgtk_embedder(pid)
        {
            ...
            match targeted {
                ...
                Ok(Err(error)) if crate::atspi::is_unverified_editable_mutation(&error) => {
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<&'static str> {
            ...
            match crate::atspi::insert_text(pid, &text) {
                ...
                Ok(false) => {}
                Err(error) if crate::atspi::is_unverified_editable_mutation(&error) => {
                    return Err(error);
                }
                Err(_) => {}
            }
...
impl Tool for MouseButtonDownTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let output_point = if desktop {
            ...
            crate::wayland::window_local_to_output(xid, xi, yi)
        } else {
            (xi, yi)
        };
        ...
        let result = if crate::wayland::is_wayland() {
            ...
            if desktop {
                tokio::task::spawn_blocking(move || {
                    crate::wayland::persistent_vptr::press_desktop(
                        &cid,
                        output_point.0,
                        output_point.1,
                        button,
            ...
            tokio::task::spawn_blocking(move || {
                crate::input::send_button_down_xtest_desktop(xi, yi, button)
            })
            .await
        } else {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        for i in 1..=steps {
            ...
            let move_result = if is_wl {
                ...
                tokio::task::spawn_blocking(move || {
                    crate::wayland::persistent_vptr::move_to(&cid_inner, motion_x, motion_y)
                })
                .await
            } else if desktop {
                tokio::task::spawn_blocking(move || {
                    crate::input::send_move_xtest_desktop(ix.round() as i32, iy.round() as i32)
                })
                .await
            } else {
                tokio::task::spawn_blocking(move || {
...
impl Tool for MouseButtonUpTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let result = if crate::wayland::is_wayland() {
            ...
            tokio::task::spawn_blocking(move || {
                crate::input::send_button_up_xtest_desktop(xi, yi, button)
            })
            .await
        } else {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let (procs, windows, mut applications) = tokio::task::spawn_blocking(move || {
            ...
            for window in &windows {
                ...
                let tree =
                    crate::atspi::walk_tree_bounded(pid, window.xid, None, max_elements, max_depth);
        ...
        for application in &mut applications {
            ...
            let snapshot_handle = cua_driver_core::element_token::token_for(snapshot_id, 0)
                .trim_end_matches(":0")
                .to_string();
                ...
                .and_then(Value::as_array_mut)
            {
                ...
                let synthetic_nodes = element_keys
                    .iter()
                    .enumerate()
                    .map(|(index, key)| crate::atspi::AtspiNode {
                ...
                for element in elements {
                    ...
                    element
                        ...
                        .remove("_element_key");
                    element["element_token"] = json!(cua_driver_core::element_token::token_for(
                        snapshot_id,
                        index as usize
                    ));
                }
...
fn window_control_def(kind: WindowControlKind) -> ToolDef {
    let contract =
        cua_driver_contract::tool_contract(kind.name()).expect("window control contract");
    ToolDef {
        name: contract.name,
        description: contract.description,
        input_schema: contract.input_schema,
...
fn normalized_wayland_control_state(
    state: crate::wayland::WindowControlState,
) -> (bool, bool, bool, bool) {
    (
        state.present,
        state.minimized,
...
fn normalized_x11_control_state(state: crate::x11::WindowControlState) -> (bool, bool, bool, bool) {
...
impl Tool for WindowControlTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let parsed = match self.kind {
            WindowControlKind::Close => cua_driver_core::tool_args::parse_typed_input::<
                cua_driver_contract::CloseWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
            WindowControlKind::Minimize => cua_driver_core::tool_args::parse_typed_input::<
                cua_driver_contract::MinimizeWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
            WindowControlKind::Maximize => cua_driver_core::tool_args::parse_typed_input::<
                cua_driver_contract::MaximizeWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
            WindowControlKind::Restore => cua_driver_core::tool_args::parse_typed_input::<
                cua_driver_contract::RestoreWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
        };
        ...
        let kind = self.kind;
        let result = if crate::wayland::is_wayland() {
            let target_exists = crate::wayland::list_windows_dispatch(Some(pid))
                .into_iter()
                .any(|window| window.xid == window_id && window.pid == Some(pid));
            if !target_exists {
                return ToolResult::error(format!(
            ...
            tokio::task::spawn_blocking(move || {
                let action = match kind {
                    ...
                    WindowControlKind::Minimize => crate::wayland::WindowControlAction::Minimize,
            ...
            tokio::task::spawn_blocking(move || {
                let action = match kind {
                    WindowControlKind::Close => crate::x11::WindowControlAction::Close,
                    WindowControlKind::Minimize => crate::x11::WindowControlAction::Minimize,
                    WindowControlKind::Maximize => crate::x11::WindowControlAction::Maximize,
                    WindowControlKind::Restore => crate::x11::WindowControlAction::Restore,
                };
                crate::x11::control_window(window_id, pid, action).map(|(before, after)| {
                    (
...
impl Tool for SetWindowFrameTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if crate::wayland::is_wayland() {
            let outcome = tokio::task::spawn_blocking(move || {
                crate::wayland::set_window_frame(window_id, pid, x, y, width, height)
