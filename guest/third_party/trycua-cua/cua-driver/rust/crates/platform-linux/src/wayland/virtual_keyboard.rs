...
pub struct Admission {
    leases: Vec<cua_driver_core::session::SessionLease>,
    shutdown_epoch: u64,
}

pub(super) fn initialize() {
    SESSION_HOOK_INSTALLED.get_or_init(|| {
        cua_driver_core::session::register_session_end_hook(end_session);
    });
...
pub(super) fn admit(
    ...
    if trusted_leases
        .iter()
        .any(|lease| !cua_driver_core::session::session_lease_is_current(lease))
    {
...
fn end_session(session: &str) {
    ...
    let active_generation = active
        ...
        .map(cua_driver_core::session::SessionLease::generation)
        .next();
    drop(active);
...
mod tests {
    ...
    fn persistent_text_rejects_oversized_requests_before_wayland() {
        let text = "x".repeat(cua_driver_contract::MAX_TYPE_TEXT_CHARS + 1);
    ...
    fn queued_waiter_with_core_lease_cannot_run_after_session_end() {
        ...
        let lease = cua_driver_core::session::capture_session_lease(session).unwrap();
        let admission = admit(vec![lease]).unwrap();
        assert!(ensure_admitted(&admission).is_ok());
        assert!(cua_driver_core::session::fire_session_end(session));
        assert!(ensure_admitted(&admission)
            .unwrap_err()
            .to_string()
            .contains("cancelled"));
        cua_driver_core::session::forget_ended_sessions_with_prefix(session);
    }
