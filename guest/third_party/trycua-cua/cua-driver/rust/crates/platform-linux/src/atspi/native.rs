...
async fn web_extents_are_physical(visited: &[Visited<'_>], pid: u32, coord: CoordType) -> bool {
    if crate::wayland::buzzardos_guest_ui_scale_120().unwrap_or(120) == 120 {
        return false;
    }
    ...
    let converted = crate::wayland::physical_rect_to_logical(x, y, width as u32, height as u32);
...
fn canonicalize_renderer_rect(
    ...
    if renderer_node_uses_physical_extents(node, physical_web_extents) {
        crate::wayland::physical_rect_to_canonical(x, y, width, height)
    } else {
        crate::wayland::logical_rect_to_canonical(x, y, width, height)
    }
}
...
fn authoritative_wayland_origin(pid: u32, xid: u64, title: Option<&str>) -> Option<(i32, i32)> {
    ...
    crate::wayland::inject_accessibility_offset(pid)
        .or_else(|| crate::wayland::compositor_ipc::window_origin_for_pid(pid))
        .or_else(|| {
            (xid != 0)
                .then(|| crate::wayland::compositor_ipc::window_for_id(xid))
                .flatten()
        ...
        .or_else(|| {
            (xid != 0)
                .then(|| {
                    crate::wayland::compositor_ipc::list_windows().and_then(|_| {
                        crate::wayland::window_geometry_logical(xid)
        ...
        .or_else(|| title.and_then(crate::wayland::compositor_ipc::window_origin_for_title))
}
...
async fn web_document_origin_for_visited(visited: &[Visited<'_>], pid: u32) -> Option<(i32, i32)> {
    ...
    let sway_window = crate::wayland::compositor_ipc::window_for_pid(pid);
