...
//!
//! This Wild Buzzard fork is Linux-only. Skill installation keeps the Linux
//! guide and shared Linux-relevant guides and rejects platform-pack expansion.
//!
//! ## Fetch source
//!
...
/// Upstream release skill tarballs contain other operating-system guides. The
/// Linux-only fork never installs those files.
fn excluded_platform_docs() -> &'static [&'static str] {
    &["WINDOWS.md", "MACOS.md", "EMBEDDING.md"]
}

/// True when the basename matches one of the excluded platform docs.
fn is_excluded_platform_doc(basename: &str) -> bool {
    let excluded = excluded_platform_docs();
...
const AGENTS: &[Agent] = &[
    ...
    // cua-driver pack symlinked here adds the Linux, recording, and browser
    // deep dives that Hermes deliberately doesn't clone.
    Agent {
        label: "Hermes",
...
fn install(flags: &[String], force: bool) -> Result<()> {
    ...
    if flags.iter().any(|flag| flag == "--all-platforms") {
        bail!("--all-platforms is unavailable in the Wild Buzzard Linux-only fork");
    }
...
fn extract_tar_gz(bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    for entry in archive.entries()? {
        ...
        // Never install non-Linux platform guides from an upstream archive.
        if let Some(basename) = stripped.file_name().and_then(|s| s.to_str()) {
            if is_excluded_platform_doc(basename) {
                continue;
            }
        }
...
mod tests {
    ...
    #[test]
    fn linux_skill_keeps_sway_and_atspi_guidance() {
        ...
        for required in ["AT-SPI", "Wayland", "Sway", "SWAYSOCK"] {
            assert!(
                linux.contains(required),
                "Linux skill lost required runtime guidance: {required}"
            );
        }
    }
    ...
    fn extract_flat_tarball_v_0_2_20_plus() {
        ...
        //   cua-driver-rs-v0.2.20-skills/LINUX.md
        let bytes = build_tarball(&[
            ("cua-driver-rs-v0.2.20-skills/SKILL.md", b"flat-skill"),
            ("cua-driver-rs-v0.2.20-skills/LINUX.md", b"flat-linux"),
        ]);
        ...
        let linux = std::fs::read_to_string(dest.path().join("LINUX.md")).unwrap();
        assert_eq!(linux, "flat-linux");
    ...
    fn extract_legacy_tarball_v_0_2_18_double_wrap_old_name() {
        ...
        let bytes = build_tarball(&[
            ...
            (
                "cua-driver-rs-v0.2.18-skills/cua-driver-rs/LINUX.md",
                b"legacy-linux",
            ),
        ]);
        let dest = tempdir().unwrap();
    ...
    #[test]
    fn extract_linux_scope_drops_non_linux_platform_docs() {
        let bytes = build_tarball(&[
        ...
        for f in [
            ...
            assert!(
                ...
                "{f} should be present after Linux-only extraction"
            );
        }
        for f in ["WINDOWS.md", "MACOS.md", "EMBEDDING.md"] {
            assert!(
                !dest.path().join(f).exists(),
                "{f} must not enter Linux scope"
            );
        }
    }
}
