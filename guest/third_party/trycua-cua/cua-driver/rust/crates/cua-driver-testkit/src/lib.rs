//! Linux test harness for cua-driver integration tests.
//!
...
//! (`send`/`call`/`init`, ×10), child-process reaping, and the
//! `result_text`/`is_error` response accessors. This crate is the single home
//! for all of it.
//!
...
//!
//! ## Child hygiene
//! [`ChildReaper`] places every spawned child in a Linux process group and
//! kills the group on drop, so tests do not leave orphaned helpers or ports.
//!
//! ## Relocated runners
//!
