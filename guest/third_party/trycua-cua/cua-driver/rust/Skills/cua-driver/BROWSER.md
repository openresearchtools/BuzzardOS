...
`get_window_state` and the normal AT-SPI/pixel action ladder in `SKILL.md`.
...
Ref- and coordinate-targeted Linux browser mutations drive the same
session-scoped agent cursor overlay as native guest-window actions.
...
If an internal Chromium page omits its web accessibility subtree, the driver
refuses generic pixel interaction. Any bounded setup fallback is limited to a
temporary tab created and navigated by the driver and must verify the exact
control transition in the same guest window.
...
Standalone Linux Chromium can activate its native window when trusted CDP
pointer input is used. CUA Driver detects that limitation and returns
`browser_input_trust_unavailable` before dispatch instead of claiming
background delivery.
...
exact page-owned dialog may still require foreground delivery. Linux Chromium
cannot resolve its native modal while preserving background posture, so the
driver refuses background mode before dispatch; retry explicitly with
...
| --- | --- | --- |
| Chrome / Chromium on Linux Xwayland | Exact binding, refs, navigation, typing, explicit DOM click | Trusted standalone click may require foreground posture |
...
| Firefox | Native guest-window state only | Typed page mutation is not supported yet |
| Tauri / other embedded webviews | Native AT-SPI/pixel fallback unless an exact route is reported | Host/renderer correlation may refuse |
