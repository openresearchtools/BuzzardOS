...
activating it, or moving the real pointer. The full Linux tool surface is
...
- The **agent cursor** is a synthetic overlay showing where the run is
  acting; it never moves the real pointer.
...
— the per-call rung of the Linux best-effort-background ladder:
...
## Linux schema contract

The capture/dispatch/addressing params follow the Linux contract in
`SKILL.md`. Live schema tests gate the `session`, `delivery_mode`,
`capture_mode`, `scope`, `modifier`, `element_index`, `snapshot_id`, and
`element_token` shapes.

- **`session` is accepted on every action/cursor tool.** Linux glides a
  per-session cursor where the overlay is available.
- **Windowless screen-absolute actions** pass `scope:"desktop"` with no
  pid/window_id. The session must have
...
2. The daemon must run **as the desktop user** (so it can read that user's
   ...
   daemon as root against a user session disconnects it from the interactive
   user's accessibility bus.
...
**A background element px action does land on X11** — for an AT-SPI app it
takes the focus-free AT-SPI `do_action`-at-point path (`x11_atspi`), exactly
as an element action. It falls to the MPX virtual-pointer path (`x11_pixel`)
only for non-accessible surfaces, **and that path
...
and the guest compositor's virtual pointer and virtual keyboard routes for
input. Neither socket is exported to the host.
...
occluded surface. Background accessibility actions can still deliver through AT-SPI, and
...
3. **Empty AT-SPI tree** (`get_window_state` returns `degraded:true`) — in
   ...
   *AT-SPI needs the session bus* above; doctor will say
   ...
   interaction or an accessibility-enable settle.

## Forbidden vectors

Do not shell out to anything that foregrounds a target:
