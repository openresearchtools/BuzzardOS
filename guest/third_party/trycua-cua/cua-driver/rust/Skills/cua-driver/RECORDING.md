...
> **Linux guest scope.** Recording uses the guest display capture route and,
> where configured, ffmpeg against the guest Xwayland display. It never records
> the surrounding host desktop.
...
**Linux video.** The recorder shells to ffmpeg with the configured Linux guest
capture source. The binary must be present in the guest; when it is missing,
per-turn capture continues without video and `last_error` reports the missing
dependency. Startup failures fail with a bounded stderr tail.
...
- `click.png` — for click-family actions (`click`, `double_click`,
  ...
  via the live AT-SPI cache, then convert to window-local screenshot
