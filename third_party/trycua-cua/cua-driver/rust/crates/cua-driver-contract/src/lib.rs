...
pub use inputs::{
    ...
    CloseWindowInput, DesktopScope, DragInput, EndSessionInput, EscalateSessionInput,
    EscalationReason, GetAgentCursorStateInput, GetCursorPositionInput, GetDesktopStateInput,
    GetScreenSizeInput, GetSessionStateInput, HotkeyInput, InvokeMenuInput, MaximizeWindowInput,
...
mod tests {
    ...
    #[test]
    fn linux_window_controls_require_one_exact_owned_window() {
        for name in [
            "close_window",
            "minimize_window",
            "maximize_window",
            ...
            let contract = tool_contract(name).expect("window control contract");
            assert_eq!(contract.platforms, vec![Platform::Linux], "{name}");
            let required = contract.input_schema["required"]
                ...
                .collect::<BTreeSet<_>>();
            assert_eq!(required, BTreeSet::from(["pid", "window_id"]), "{name}");
        }
    }
