//! Non-Linux stubs — return a helpful "not implemented" message.
...
fn not_impl(name: &str) -> ToolResult {
    ToolResult::error(format!(
        "{name}: Linux implementation is only available when compiled for Linux (target_os = \"linux\")."
    ))
}
...
stub_tool!(
    ...
    GetScreenSizeTool,
    "get_screen_size",
    "Return the main screen's canonical physical width and height.",
    serde_json::json!({"type":"object","properties":{},"additionalProperties":false})
);
