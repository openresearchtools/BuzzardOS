...
pub fn parse_typed_projection<T: crate::contract::ToolInput>(
    tool_name: &str,
    args: &Value,
) -> Result<T, ToolResult> {
    ...
    let properties = crate::contract::tool_input_fields(T::TOOL_NAME)
...
mod reserved_args_tests {
    ...
    use crate::contract::{ClickButton, ClickInput, GetScreenSizeInput, SetWindowFrameInput};
    use serde_json::json;
    ...
    fn typed_input_ignores_trusted_transport_metadata_but_rejects_public_unknown_fields() {
        ...
        assert!(matches!(
            &error.content[0],
            crate::core::protocol::Content::Text { text, .. }
