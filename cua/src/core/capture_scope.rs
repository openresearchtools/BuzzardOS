...
impl SessionCaptureScope {
    ...
    pub fn output(&self, session: &str) -> crate::contract::SessionStateOutput {
        crate::contract::SessionStateOutput {
...
pub fn bind_session(
    ...
    if crate::core::session::is_session_ended(session) {
...
mod tests {
    ...
    fn end_racing_escalation_never_resurrects_scope_state() {
        ...
        let end = std::thread::spawn(move || {
            end_barrier.wait();
            crate::core::session::fire_session_end(&end_session)
        });
        barrier.wait();
        let _ = escalation.join().unwrap();
        assert!(end.join().unwrap());

        assert!(crate::core::session::is_session_ended(&session));
        assert!(get_session(&session).is_none());
        assert_eq!(
        ...
        assert!(crate::core::session::revive_session(&session));
