...
pub enum ActionTransport {
    ...
    LinuxWaylandForeignToplevel,
    LinuxWaylandCompositorIpc,
    LinuxX11ConfigureWindow,
    LinuxX11WindowManager,
...
impl ActionTransport {
    pub const ALL: &'static [Self] = &[
        ...
        Self::LinuxWaylandForeignToplevel,
        Self::LinuxWaylandCompositorIpc,
        Self::LinuxX11ConfigureWindow,
        Self::LinuxX11WindowManager,
    ...
    pub const fn route(self) -> ActionRoute {
        match self {
            ...
            | Self::LinuxWaylandCompositorIpc
            | Self::LinuxX11ConfigureWindow
            | Self::LinuxX11WindowManager => ActionRoute::SystemApi,
...
impl ActionExecutionRecord {
    ...
    pub fn public_result(
        &self,
    ) -> Result<crate::contract::ActionResult, ActionRecordValidationError> {
        let projection = self.stable_projection()?;
        Ok(crate::contract::ActionResult {
            effect: match projection.effect {
                ActionEffect::Confirmed => crate::contract::ActionEffect::Confirmed,
                ActionEffect::Partial => crate::contract::ActionEffect::Partial,
                ActionEffect::Unverifiable => crate::contract::ActionEffect::Unverifiable,
                ActionEffect::SuspectedNoop => crate::contract::ActionEffect::SuspectedNoop,
                ActionEffect::Refused => crate::contract::ActionEffect::Refused,
            },
            route: match projection.route {
                ActionRoute::Accessibility => crate::contract::ActionRoute::Accessibility,
                ActionRoute::SyntheticEvents => crate::contract::ActionRoute::SyntheticEvents,
                ActionRoute::GlobalInput => crate::contract::ActionRoute::GlobalInput,
                ActionRoute::SystemApi => crate::contract::ActionRoute::SystemApi,
                ActionRoute::Dom => crate::contract::ActionRoute::Dom,
                ActionRoute::TrustedInput => crate::contract::ActionRoute::TrustedInput,
            },
            delivery: projection
                .delivery
                .map(|delivery| crate::contract::ActionDelivery {
                    mode: match delivery.actual {
                        ActualDelivery::Background => {
                            crate::contract::ActionDeliveryMode::Background
                        }
                        ActualDelivery::Foreground => {
                            crate::contract::ActionDeliveryMode::Foreground
                        }
                        ActualDelivery::NotApplicable => {
                            crate::contract::ActionDeliveryMode::NotApplicable
                        }
                        ActualDelivery::Unknown => crate::contract::ActionDeliveryMode::Unknown,
                    },
            ...
            evidence: projection.evidence.map(|evidence| {
                evidence
                    .into_iter()
                    .map(|evidence| crate::contract::ActionEvidence {
                        kind: match evidence.kind {
                            ...
                            | ProjectedEvidenceKind::ValueReadback => {
                                crate::contract::ActionEvidenceKind::ValueReadback
                            }
                            ProjectedEvidenceKind::WindowChange => {
                                crate::contract::ActionEvidenceKind::WindowChange
                            }
                            ProjectedEvidenceKind::ScreenshotComparison => {
                                crate::contract::ActionEvidenceKind::ScreenshotChange
                            }
                        },
                    })
                    .collect()
    ...
    pub fn from_legacy(
        ...
        if let Some(kind) = legacy_publishable_evidence_kind(tool_name, structured, transport) {
...
pub fn is_action_tool(tool_name: &str) -> bool {
    crate::contract::is_action_result_tool(tool_name)
}
...
fn legacy_publishable_evidence_kind(
    ...
    let input_state_readback = matches!(
        ...
        .is_some_and(|evidence| {
            evidence.iter().any(|item| {
                item.get("kind").and_then(serde_json::Value::as_str) == Some("value_readback")
            })
        });
    ...
    if screenshot_comparison {
        return Some(EvidenceKind::ScreenshotComparison);
    }
    if input_state_readback {
        return Some(EvidenceKind::ValueReadback);
    }
    if !(value_readback_tool || click_selection_readback) {
        return None;
    }
    Some(
        if matches!(
    ...
    let evidence: Vec<_> = evidence
        ...
        .filter_map(|evidence| {
            let kind = match evidence.kind {
                ...
                EvidenceKind::ScreenshotComparison => ProjectedEvidenceKind::ScreenshotComparison,
...
fn transport_name(transport: ActionTransport) -> &'static str {
    match transport {
        ...
        ActionTransport::LinuxWaylandForeignToplevel => "linux_wayland_foreign_toplevel",
        ActionTransport::LinuxWaylandCompositorIpc => "linux_wayland_compositor_ipc",
        ActionTransport::LinuxX11ConfigureWindow => "linux_x11_configure_window",
        ActionTransport::LinuxX11WindowManager => "linux_x11_window_manager",
...
mod tests {
    ...
    #[test]
    fn stateful_pointer_hold_readback_is_publishable_value_evidence() {
        let structured = serde_json::json!({
            "verified": true,
            "effect": "confirmed",
            "evidence": [{"kind": "value_readback"}],
        });
        for tool in ["mouse_button_down", "mouse_drag", "mouse_button_up"] {
            assert_eq!(
                legacy_publishable_evidence_kind(
                    tool,
                    &structured,
                    ActionTransport::LinuxWaylandVirtualPointer,
                ),
                Some(EvidenceKind::ValueReadback),
            );
        }
    }
    ...
    #[test]
    fn projection_publishes_native_screenshot_change_evidence() {
        let record = ActionExecutionRecord::builder(
            ActionEffect::Unverifiable,
            ActionTransport::MacosCgEventHid,
            RequestedDelivery::Foreground,
        ...
        assert_eq!(
            record.stable_projection().unwrap().evidence,
            Some(vec![ActionEvidenceProjection {
                kind: ProjectedEvidenceKind::ScreenshotComparison,
                detail: "pixels changed".to_owned(),
            }])
        );
    }
    ...
    fn escalation_projection_preserves_decision_critical_reasons() {
        use crate::contract::{ActionEscalationReason, ActionEscalationTarget};
        ...
        for (kind, target, reason) in cases {
            ...
            assert_eq!(
                result.escalation,
                Some(crate::contract::ActionEscalation { target, reason })
            );
        }
    ...
    #[test]
    fn confirmed_accepts_native_screenshot_change_evidence() {
        let record = ActionExecutionRecord::builder(
            ActionEffect::Confirmed,
            ActionTransport::MacosCgEventHid,
            RequestedDelivery::Foreground,
        ...
        assert_eq!(
            record.evidence,
            Some(vec![crate::contract::ActionEvidence {
                kind: crate::contract::ActionEvidenceKind::ScreenshotChange,
            }])
        );
    }
    ...
    fn browser_pointer_routes_share_the_closed_action_contract() {
        for (input_route, expected_route) in [
            ("trusted", crate::contract::ActionRoute::TrustedInput),
            ("dom_event", crate::contract::ActionRoute::Dom),
        ] {
            let args = serde_json::json!({
            ...
            assert_eq!(
                public.effect,
                crate::contract::ActionEffect::Unverifiable
            );
            assert_eq!(public.route, expected_route);
            assert_eq!(
                public.delivery.as_ref().map(|delivery| delivery.mode),
                Some(crate::contract::ActionDeliveryMode::Background)
            );
    ...
    fn browser_refusals_and_partial_delivery_survive_projection() {
        ...
        assert_eq!(refused.effect, crate::contract::ActionEffect::Refused);
        assert!(refused.delivery.is_none());
        assert_eq!(
            refused.escalation,
            Some(crate::contract::ActionEscalation {
                target: crate::contract::ActionEscalationTarget::Page,
                reason: crate::contract::ActionEscalationReason::RouteUnavailable,
            })
        );
        ...
        assert_eq!(partial.effect, crate::contract::ActionEffect::Partial);
        assert_eq!(
            partial
                .delivery
                .as_ref()
    ...
    fn successful_foreground_pixel_branch_reports_actual_global_delivery() {
        ...
        assert_eq!(
            ...
            Some(crate::contract::ActionDeliveryMode::Foreground)
        );
        assert_eq!(public.route, crate::contract::ActionRoute::GlobalInput);
    }
