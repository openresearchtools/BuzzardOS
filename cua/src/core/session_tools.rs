...
//! [`crate::core::session`]). It owns the agent cursor and is the key for
...
impl Tool for StartSessionTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let was_ended = crate::core::session::is_session_ended(&id);
        if !was_ended {
        ...
        let revived = crate::core::session::revive_session(&id);
        ...
        crate::core::session::touch_session(&id);
        let structured = serde_json::to_value(crate::contract::StartSessionOutput {
...
impl Tool for EndSessionTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        crate::core::session::end_session(&id);
        ToolResult::text(format!("✅ Session '{id}' ended.")).with_structured(
            serde_json::to_value(crate::contract::EndSessionOutput {
                session: id,
                active: false,
            })
