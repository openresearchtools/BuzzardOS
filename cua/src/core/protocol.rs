...
pub struct ToolResult {
    ...
    #[serde(skip)]
    pub action_record: Option<crate::core::action_record::ActionExecutionRecord>,
}
...
impl ToolResult {
    ...
    pub fn with_action_record(
        mut self,
        record: crate::core::action_record::ActionExecutionRecord,
    ) -> Self {
...
fn agent_instructions() -> String {
    String::from(
        r#"cua-driver: Linux Sway/Wayland/Xwayland computer-use automation.
...
For an app or window outcome, use the narrowest semantic Cua route first: `set_window_frame` plus `list_windows` readback for geometry, typed browser tools for supported page content, and clipboard tools for clipboard state. Then climb through background `element_index` (AT-SPI), background pixels, foreground delivery, and desktop fallback. Never advance on transport success alone.
...
If the `cua-driver` skill is loaded, follow SKILL.md plus LINUX.md."#,
    )
}
