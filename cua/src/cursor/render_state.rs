...
impl RenderStateCore {
    ...
    pub fn update_session_badge_hover(&mut self, pointer: Option<(f64, f64)>) -> bool {
        const HOVER_RADIUS: f64 = crate::cursor::theme::DISPLAY_SIZE as f64 * 0.82;
        let hovered = self.session_badge_needs_hover_poll()
            && pointer.is_some_and(|(x, y)| {
                ...
                crate::cursor::session_badge_layout(crate::cursor::SessionBadgeInput {
    ...
    pub fn apply_command_base(
        ...
        match cmd {
            ...
            OverlayCommand::SetSessionLabel(label) => {
                let session_label = crate::cursor::sanitize_session_label(&label);
...
pub fn paint_cursor(
    ...
    if let Some(layout) = crate::cursor::session_badge_layout(crate::cursor::SessionBadgeInput {
        ...
        crate::cursor::paint_session_badge(
            pm,
            &layout,
            crate::cursor::session_fill_rgba(&core.cfg.cursor_id),
            alpha_scale,
...
mod session_badge_and_action_tests {
    use super::*;
    use crate::cursor::{CursorConfig, DeliveryModifier, TargetModifier};
