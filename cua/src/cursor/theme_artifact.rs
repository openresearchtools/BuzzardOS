...
const FPS: u16 = 30;
const DEFAULT_THEME_BYTES: &[u8] = include_bytes!("../../assets/cursor/cua.default.cua-theme");
...
fn resolved_color(mut color: [u8; 4], tint: Option<[u8; 4]>) -> [u8; 4] {
    if color[0..3] == crate::cursor::theme::DEFAULT_CURSOR_FILL[0..3] {
...
pub fn paint_compiled_theme_with_tint(
    ...
    let reduced = visual.reduced_motion == crate::cursor::ReducedMotion::On;
...
mod tests {
    ...
    fn full_profile_requires_every_action() {
        ...
        theme.profile = crate::cursor::THEME_PROFILE.into();
    ...
    fn embedded_default_is_a_full_compiled_theme() {
        ...
        assert_eq!(theme.id, crate::cursor::DEFAULT_THEME_ID);
        ...
        assert!(load_installed_theme(crate::cursor::DEFAULT_THEME_ID)
            .unwrap()
            .is_some());
    }
