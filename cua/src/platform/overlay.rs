...
//!
//! Animation state + render pipeline live in `crate::cursor::render_state`
...
use crate::cursor::CursorAction;
#[cfg(target_os = "linux")]
use crate::cursor::ZOrderEnforcer;
use crate::cursor::{
...
pub fn init(cfg: CursorConfig) {
    ...
    crate::core::cursor_events::install_cursor_event_sink(std::sync::Arc::new(
        |event: crate::core::cursor_events::CursorEvent| {
            use crate::core::cursor_events::{CursorEvent, CursorEventPhase};
...
pub fn send_command_for(key: CursorKey, cmd: OverlayCommand) {
    ...
    // Forward directly to the private Sway layer-shell overlay.
    #[cfg(target_os = "linux")]
    {
        if crate::platform::wayland::is_wayland() {
            let _ = crate::platform::wayland::overlay::forward(&msg);
        }
    }
}
...
pub fn current_motion_for(key: &str) -> crate::cursor::MotionConfig {
    RENDER
        .lock()
        .ok()
        .and_then(|guard| {
...
pub fn current_theme_state_for(
    ...
    String,
    Option<String>,
    crate::cursor::CursorVisualState,
)> {
...
// in `crate::cursor::render_state` (2026-05 dedup audit). What stays here
...
// ── Z-order enforcer (Linux impl of crate::cursor::ZOrderEnforcer) ──────

/// X11 implementation of [`crate::cursor::ZOrderEnforcer`].
///
...
const X11_BADGED_CURSOR_HORIZONTAL_MARGIN: f64 =
    crate::cursor::session_badge_extents().horizontal as f64 + 2.0;
...
mod tests {
            ...
            let (arrived, had_msg) = process_render_wake(
                ...
                Some(OverlayMsg::Cmd(KeyedOverlayCommand {
                    ...
                    cmd: OverlayCommand::SetTheme {
                        theme_id: crate::cursor::DEFAULT_THEME_ID.to_owned(),
                        reduced_motion: crate::cursor::ReducedMotion::Auto,
                    },
                })),
                &rx,
                0.1,
    ...
    fn modifier_only_badge_expands_the_local_cursor_tile() {
        ...
        cursor.apply_command(OverlayCommand::BeginAction {
            ...
            delivery: Some(crate::cursor::DeliveryModifier::Foreground),
            target: Some(crate::cursor::TargetModifier::Pixel),
        });
