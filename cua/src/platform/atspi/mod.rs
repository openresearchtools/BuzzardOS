...
pub struct AtspiNode {
    ...
    /// For AT-SPI: a stable hash of the exporting D-Bus name and object path.
...
/// Associate one opaque CUA snapshot with the stable AT-SPI object identities
/// observed by that exact walk. Bounded Chromium/Electron walks can omit deep
/// descendants, so their dense element ordinals must never be resolved against
/// a later unbounded walk.
pub fn register_snapshot_element_keys(pid: u32, snapshot_id: &str, nodes: &[AtspiNode]) {
    let keys = nodes
        .iter()
...
pub fn perform_action_by_key(pid: u32, key: u64) -> Result<(String, bool)> {
    native::perform_action_by_key(pid, key)
}
...
pub(crate) fn is_unverified_editable_mutation(error: &anyhow::Error) -> bool {
    native::is_unverified_editable_mutation(error)
}
