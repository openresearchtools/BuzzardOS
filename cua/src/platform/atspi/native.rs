...
use atspi::{CoordType, Interface, Role, State};
...
fn renderer_node_uses_physical_extents(node: &Visited<'_>, physical_web_extents: bool) -> bool {
    physical_web_extents && (node.in_web_doc || is_document_role(&node.role))
}
...
fn rect_fits_window(
    ...
    i64::from(x) >= -TOLERANCE
        && i64::from(y) >= -TOLERANCE
        && right <= i64::from(window_width) + TOLERANCE
        && bottom <= i64::from(window_height) + TOLERANCE
}

/// Detect Chromium/Electron's mixed AT-SPI coordinate space.
///
/// At fractional output scale Chromium's browser chrome reports logical
/// `CoordType::Window` extents, while the renderer's `document web` subtree
/// reports physical-buffer pixels. The shallowest document is the renderer
/// viewport, so it must fit the compositor window after conversion. Requiring
...
async fn web_extents_are_physical(visited: &[Visited<'_>], pid: u32, coord: CoordType) -> bool {
    if crate::platform::wayland::buzzardos_guest_ui_scale_120().unwrap_or(120) == 120 {
        return false;
    }
    let Some(window) = crate::platform::wayland::compositor_ipc::window_for_pid(pid) else {
        return false;
    };
    let Some(document) = visited
        .iter()
        .filter(|node| node.has_component && is_document_role(&node.role))
        .min_by_key(|node| node.depth)
    else {
        return false;
    };
    let Some(Ok(proxies)) = call(document.acc.proxies()).await else {
        return false;
    };
    let Some(Ok(component)) = call(proxies.component()).await else {
        return false;
    };
    let Some(Ok((x, y, width, height))) = call(component.get_extents(coord)).await else {
        return false;
    };
    if width <= 1 || height <= 1 {
        return false;
    }
    let raw_fits = rect_fits_window(
        x,
        y,
        width as u32,
        height as u32,
    ...
    let physical = !raw_fits && converted_fits;
    dlog!(
        "web extent coordinate probe: raw=({x},{y},{width},{height}) \
         converted={converted:?} window={}x{} physical={physical}",
        window.width,
        window.height
    );
    physical
...
fn object_ref_key(oref: &RawObjectRef) -> u64 {
    // Stable FNV-1a over the complete AT-SPI identity. The key never leaves
    // the CUA process and is verified against a live re-walk before use. It
    // lets a snapshot token keep targeting the same accessible when a bounded
    // walk omitted descendants that would otherwise shift ordinal indices.
    ...
    for byte in oref
        ...
        .chain(oref.path.as_bytes().iter().copied())
    {
        hash ^= u64::from(byte);
...
async fn collect_visited_bounded<'a>(
    ...
    while let Some((oref, depth, inherited_web_doc)) = stack.pop() {
        ...
        let object_key = object_ref_key(&oref);
        ...
        let (localized_role_r, role_r, name_r, state_r, children_r) = tokio::join!(
        ...
        // `GetRoleName` is optional in AT-SPI and AccessKit's bridge exposes
        // the standard numeric `GetRole` method without the historical
        // `GetRoleName` alias. Falling back to the enum's canonical English
        // name keeps native Rust/AccessKit controls semantically typed.
        let localized_role = match localized_role_r {
            Some(Ok(role)) => role,
            _ => String::new(),
        };
        let role = canonical_role_name(
            &localized_role,
            match role_r {
...
pub fn list_windows(filter_pid: Option<u32>) -> Vec<crate::platform::x11::WindowInfo> {
...
fn list_windows_blocking(filter_pid: Option<u32>) -> Vec<crate::platform::x11::WindowInfo> {
...
async fn write_through_editable_proxies(
    ...
    // AT-SPI EditableText.InsertText follows ATK's C ABI: `length` is the
    // number of UTF-8 bytes, while `position` and Text-interface ranges are
    // Unicode character offsets. Passing `chars().count()` truncates whatever
    // follows a multibyte character (for example an em dash consumes three
    // bytes but only one character).
    let len = editable_text_byte_length(text);

    if et.insert_text(off, text, len).await.unwrap_or(false) {
        if editable_text_range_matches(proxies, off, text).await? {
            return Ok(true);
        }
        return Err(EditableMutationUnverified {
            detail: format!(
                "InsertText acknowledged {len} UTF-8 byte(s) at character offset {off}, \
                 but exact AT-SPI range readback did not match"
            ),
        }
        .into());
    }
    if et.set_text_contents(text).await.unwrap_or(false) {
        if editable_text_contents_match(proxies, text).await? {
            return Ok(true);
        }
        return Err(EditableMutationUnverified {
            detail:
                "SetTextContents was acknowledged, but exact AT-SPI contents readback did not match"
                    .into(),
        }
        .into());
    }
...
#[error("AT-SPI editable mutation is not safely retryable: {detail}")]
struct EditableMutationUnverified {
    detail: String,
}

pub(super) fn is_unverified_editable_mutation(error: &anyhow::Error) -> bool {
    error.downcast_ref::<EditableMutationUnverified>().is_some()
}

fn editable_text_byte_length(text: &str) -> i32 {
    text.len().min(i32::MAX as usize) as i32
}

async fn editable_text_range_matches(
    proxies: &atspi::proxy::proxy_ext::Proxies<'_>,
    start: i32,
    expected: &str,
) -> Result<bool> {
    let text_proxy = proxies
        .text()
        .await
        .map_err(|error| anyhow!("Text unavailable for exact editable readback: {error}"))?;
    let expected_characters = expected.chars().count().min(i32::MAX as usize) as i32;
    let end = start.saturating_add(expected_characters);
    for attempt in 0..5 {
        if text_proxy.get_text(start, end).await.unwrap_or_default() == expected {
            return Ok(true);
        }
        if attempt < 4 {
...
async fn editable_text_contents_match(
    proxies: &atspi::proxy::proxy_ext::Proxies<'_>,
    expected: &str,
) -> Result<bool> {
    let text_proxy = proxies
    ...
    let expected_characters = expected.chars().count().min(i32::MAX as usize) as i32;
    for attempt in 0..5 {
        let count = text_proxy.character_count().await.unwrap_or(-1);
        if count == expected_characters
            && text_proxy.get_text(0, count).await.unwrap_or_default() == expected
        {
            return Ok(true);
        }
        if attempt < 4 {
...
async fn entry_find_window_xid(pid: u32) -> Option<u64> {
    use crate::platform::x11::list_windows;
...
async fn actuate_visited(target: &Visited<'_>, description: &str) -> Result<(String, bool)> {
    ...
    let chosen = activation_index(&target.role, &target.actions)
        .ok_or_else(|| anyhow!("{description} does not advertise a safe activation action"))?;
    ...
    if !accepted {
        anyhow::bail!("{description} rejected its AT-SPI {action:?} action");
    }
    // AT-SPI's acknowledgement can precede the renderer's queued DOM mutation.
...
pub fn perform_action(pid: u32, idx: usize) -> Result<(String, bool)> {
    bounded(
        async {
            ...
            actuate_visited(target, &format!("element {idx}")).await
        },
        || {
            Err(anyhow!(
...
pub fn perform_action_by_key(pid: u32, key: u64) -> Result<(String, bool)> {
    bounded(
        async {
            let conn = shared_connection().await?;
            let visited = collect_visited(conn, pid)
            ...
            let target = visited
                .iter()
                .find(|candidate| is_indexable(candidate) && candidate.object_key == key)
                .ok_or_else(|| anyhow!("snapshot AT-SPI element is no longer present"))?;
            actuate_visited(target, "snapshot AT-SPI element").await
        },
        || {
            Err(anyhow!(
                "perform_action_by_key timed out for pid {pid} (app unresponsive to AT-SPI)"
            ))
        },
    )
}
        ...
        async {
            ...
            if let Ok(et) = proxies.editable_text().await {
                ...
                if et.set_text_contents(value).await.unwrap_or(false) {
                    if editable_text_contents_match(&proxies, value).await? {
                        return Ok(());
                    }
                    return Err(EditableMutationUnverified {
                        detail: "SetTextContents was acknowledged, but exact AT-SPI contents readback did not match"
                            .into(),
                    }
                    .into());
                }
                ...
                let len = editable_text_byte_length(value);
                if et.insert_text(off, value, len).await.unwrap_or(false) {
                    if editable_text_range_matches(&proxies, off, value).await? {
                        return Ok(());
                    }
                    return Err(EditableMutationUnverified {
                        detail: format!(
                            "InsertText acknowledged {len} UTF-8 byte(s) at character offset \
                             {off}, but exact AT-SPI range readback did not match"
                        ),
                    }
                    .into());
                }
...
pub fn get_element_bounds(pid: u32, idx: usize) -> Result<(i32, i32, u32, u32)> {
    bounded(
        async {
            ...
            let offset = window_to_screen_offset(pid, 0, None);
            let coord = if offset.is_some() {
                CoordType::Window
            } else {
                CoordType::Screen
...
fn window_to_screen_offset(pid: u32, xid: u64, title: Option<&str>) -> Option<(i32, i32)> {
    if crate::platform::wayland::is_wayland() {
        ...
        return prefer_authoritative_wayland_origin(
            authoritative,
            crate::platform::wayland::observed_window_origin(pid),
        );
    }
...
fn authoritative_wayland_origin(pid: u32, xid: u64, title: Option<&str>) -> Option<(i32, i32)> {
    ...
    crate::platform::wayland::compositor_ipc::window_origin_for_pid(pid)
        ...
        .or_else(|| {
            (xid != 0)
                .then(|| {
                    crate::platform::wayland::compositor_ipc::list_windows().and_then(|_| {
                        crate::platform::wayland::window_geometry_logical(xid)
...
async fn web_document_origin_for_visited(visited: &[Visited<'_>], pid: u32) -> Option<(i32, i32)> {
    ...
    let sway_window = crate::platform::wayland::compositor_ipc::window_for_pid(pid);
...
async fn element_bounds_for_visited(
    ...
    let screen_rebase = if offset.is_none() && !crate::platform::wayland::is_wayland() && xid != 0 {
...
mod coord_tests {
    ...
    fn numeric_role_fills_missing_optional_localized_role_name() {
        assert_eq!(canonical_role_name("", Some(Role::Button)), "button");
        assert_eq!(
            canonical_role_name("localized button", Some(Role::Button)),
            "localized button"
        );
        assert_eq!(canonical_role_name("", None), "");
    }
    ...
    #[test]
    fn editable_text_lengths_follow_the_atk_utf8_byte_contract() {
        assert_eq!(editable_text_byte_length("plain"), 5);
        assert_eq!(editable_text_byte_length("—"), 3);
        assert_eq!(editable_text_byte_length("a—b"), 5);
        assert_eq!(editable_text_byte_length("café"), 5);
        assert_eq!(editable_text_byte_length("日本語"), 9);
    }
