...
//! `crate::platform::tty`. We deliberately do NOT fall back to the XTest extension for
...
fn send_button_xtest_desktop(x: i32, y: i32, button: u8, pressed: bool) -> Result<()> {
    use x11rb::protocol::xtest::ConnectionExt as _;
    let (conn, screen_num) = connect_x11_for_input()?;
    let root = conn.setup().roots[screen_num].root;
    conn.xtest_fake_input(MOTION_NOTIFY_EVENT, 0, 0, root, x as i16, y as i16, 0)?;
...
/// Hold one X11 pointer button at a canonical desktop coordinate.
pub fn send_button_down_xtest_desktop(x: i32, y: i32, button: u8) -> Result<()> {
    send_button_xtest_desktop(x, y, button, true)
}

/// Release one X11 pointer button at a canonical desktop coordinate.
pub fn send_button_up_xtest_desktop(x: i32, y: i32, button: u8) -> Result<()> {
    send_button_xtest_desktop(x, y, button, false)
}
