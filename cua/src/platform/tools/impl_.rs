...
fn keyboard_admission(
    _args: &Value,
) -> Result<crate::platform::wayland::KeyboardAdmission, ToolResult> {
    crate::platform::wayland::keyboard_admission()
...
fn guest_output_frames_equal_masked(
    before: &[u8],
    after: &[u8],
    cursor_masks: &[(i32, i32)],
) -> Option<bool> {
    let before = image::load_from_memory(before).ok()?.into_rgba8();
    let after = image::load_from_memory(after).ok()?.into_rgba8();
    if before.dimensions() != after.dimensions() {
        return Some(false);
    }
    let (width, _) = before.dimensions();
    Some(
        before
            .as_raw()
            .chunks_exact(4)
            ...
            .all(|(index, (before, after))| {
                ...
                point_near_cursor_path(x, y, cursor_masks) || before == after
            }),
    )
}

fn guest_output_difference_summary(
    before: &[u8],
    after: &[u8],
    cursor_masks: &[(i32, i32)],
) -> Option<Value> {
        ...
        .enumerate()
    {
        ...
        if before != after && !point_near_cursor_path(x as i32, y as i32, cursor_masks) {
            changed_pixels += 1;
            min_x = min_x.min(x);
            min_y = min_y.min(y);
            max_x = max_x.max(x);
    ...
    Some(json!({
        ...
        "changed_bounds": (changed_pixels > 0).then(|| {
            json!({
                "x": min_x,
                "y": min_y,
                "width": max_x - min_x + 1,
...
fn visual_observation_payload_masked(
    path: &str,
    before: Option<&[u8]>,
    after: Option<&[u8]>,
    cursor_masks: &[(i32, i32)],
    ...
    match (before, after) {
        (Some(before), Some(after))
            if guest_output_frames_equal_masked(before, after, cursor_masks) == Some(false) =>
        {
            let mut result = json!({
                "path": path,
                "verified": true,
                "effect": "confirmed",
                "verify": "native full-output screenshot changed after the action",
        ...
        (Some(before), Some(after))
            if guest_output_frames_equal_masked(before, after, cursor_masks) == Some(true) =>
        {
            json!({
                "path": path,
                "verified": false,
...
fn visual_observation_payload(path: &str, before: Option<&[u8]>, after: Option<&[u8]>) -> Value {
    visual_observation_payload_masked(path, before, after, &[])
}

async fn capture_guest_output() -> Option<Vec<u8>> {
    tokio::task::spawn_blocking(crate::platform::capture::screenshot_display_bytes)
        .await
        .ok()
        .and_then(Result::ok)
}

async fn capture_guest_output_after_pointer_at(
    x: i32,
    y: i32,
) -> (Option<Vec<u8>>, Vec<(i32, i32)>) {
    let previous_pointer = if crate::platform::wayland::wayland_input_enabled() {
        crate::platform::wayland::last_synth_cursor_pos()
    } else {
        None
    };
    let mut cursor_masks = previous_pointer.into_iter().collect::<Vec<_>>();
    if !cursor_masks.contains(&(x, y)) {
        cursor_masks.push((x, y));
    }
    // A nested wlroots backend may composite its software cursor into the
    // output even when zwlr_screencopy's overlay_cursor flag is false. Move the
    // real guest pointer before taking the baseline so the action itself does
    // not create a cursor-only frame difference.
    let positioned = tokio::task::spawn_blocking(move || {
        if crate::platform::wayland::wayland_input_enabled() {
            crate::platform::wayland::move_cursor_absolute(None, x, y)
        } else {
            crate::platform::input::send_move_xtest_desktop(x, y)
        }
    })
    .await
    .ok()
    ...
    .is_some();
    if !positioned {
        return (None, cursor_masks);
    }

    // Allow the compositor to commit the warp. The animated cursor footprint
    // is masked during comparison, so the entire frame need not become static.
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    (capture_guest_output().await, cursor_masks)
}

async fn capture_guest_output_with_current_pointer_mask() -> (Option<Vec<u8>>, Vec<(i32, i32)>) {
    let cursor_masks = if crate::platform::wayland::wayland_input_enabled() {
        crate::platform::wayland::last_synth_cursor_pos()
            .into_iter()
            .collect()
    } else {
...
async fn observe_guest_output_change(before: Option<Vec<u8>>, path: &str) -> Value {
    // Wait for the compositor transaction and the shell/application repaint
    // that follows it. This is short enough for interactive use and bounded;
    // native capture remains the authoritative guest-only output observation.
    tokio::time::sleep(std::time::Duration::from_millis(120)).await;
    let after = capture_guest_output().await;
    visual_observation_payload(path, before.as_deref(), after.as_deref())
}

async fn observe_guest_output_change_masked(
    ...
    // frame exists. Poll the real guest output for a bounded interval instead
...
async fn observe_typed_text(
    before: Option<Vec<u8>>,
    cursor_masks: &[(i32, i32)],
    path: &str,
    characters: usize,
) -> Value {
    let mut observation = observe_guest_output_change_masked(before, path, cursor_masks).await;
    observation["path"] = json!(path);
    observation["characters"] = json!(characters);
    observation["verified"] =
        json!(observation.get("effect").and_then(Value::as_str) == Some("confirmed"));
    if observation.get("effect").and_then(Value::as_str) != Some("confirmed") {
        observation["escalation"] = json!({
            "recommended": "observe_and_retry",
            "reason": "the compositor accepted the key sequence but no text change was \
                       observed in the guest output"
        });
    }
    observation
}
...
#[cfg(test)]
mod visual_observation_tests {
    use super::{visual_observation_payload, visual_observation_payload_masked};
    use image::{DynamicImage, ImageFormat, Rgba, RgbaImage};
    use std::io::Cursor;

    fn frame(pixel: [u8; 4]) -> Vec<u8> {
        let image = DynamicImage::ImageRgba8(RgbaImage::from_pixel(2, 2, Rgba(pixel)));
        let mut bytes = Cursor::new(Vec::new());
        image.write_to(&mut bytes, ImageFormat::Png).unwrap();
        bytes.into_inner()
    }

    fn frame_with_changed_pixel(x: u32, y: u32) -> Vec<u8> {
        let mut image = RgbaImage::from_pixel(256, 256, Rgba([0, 0, 0, 255]));
    ...
    #[test]
    fn changed_native_capture_is_confirmed_with_explicit_evidence() {
        let before = frame([0, 0, 0, 255]);
        let after = frame([255, 0, 0, 255]);
        let payload = visual_observation_payload(
            "wayland_desktop",
            Some(before.as_slice()),
            Some(after.as_slice()),
        );
        assert_eq!(payload["effect"], "confirmed");
        assert_eq!(payload["verified"], true);
        assert_eq!(payload["evidence"][0]["kind"], "screenshot_comparison");
    }

    #[test]
    fn encoded_byte_differences_with_identical_pixels_are_not_visual_evidence() {
        let before = frame([0, 0, 0, 255]);
        let mut after = before.clone();
        after.extend_from_slice(b"ignored trailing bytes");
        let unchanged = visual_observation_payload(
            "wayland_desktop",
            Some(before.as_slice()),
            Some(after.as_slice()),
        );
        assert_eq!(unchanged["effect"], "suspected_noop");
        assert_eq!(unchanged["verified"], false);
    }

    #[test]
    fn missing_or_invalid_capture_never_claims_confirmation() {
        let missing = visual_observation_payload("wayland_desktop", None, Some(b"after"));
        assert_eq!(missing["effect"], "unverifiable");
        assert_eq!(missing["verified"], false);

        let invalid =
            visual_observation_payload("wayland_desktop", Some(b"before"), Some(b"after"));
        assert_eq!(invalid["effect"], "unverifiable");
        assert_eq!(invalid["verified"], false);
    }

    #[test]
    fn cursor_footprint_changes_are_ignored_but_other_pixels_still_confirm() {
        let before = frame_with_changed_pixel(0, 0);
        let cursor_only = frame_with_changed_pixel(100, 100);
        let masked = visual_observation_payload_masked(
            "wayland_desktop",
            Some(before.as_slice()),
            Some(cursor_only.as_slice()),
            &[(100, 100), (0, 0)],
        );
        assert_eq!(masked["effect"], "suspected_noop");

        let outside = frame_with_changed_pixel(240, 240);
...
fn window_target_candidates_for_pid(
    windows: impl IntoIterator<Item = crate::platform::x11::WindowInfo>,
    pid: u32,
...
fn pid_window_target_candidates(pid: i64) -> Vec<WindowTargetCandidate> {
    ...
    window_target_candidates_for_pid(
...
fn config_path() -> std::path::PathBuf {
    ...
    root.join("buzzardoscua").join("config.json")
}
...
fn write_config_file(config: &DriverConfig) -> anyhow::Result<()> {
...
/// Load persistent daemonless CLI configuration, falling back to defaults.
pub fn load_driver_config() -> DriverConfig {
    let mut cfg = DriverConfig::default();
    let saved = read_config_file();
    if let Some(value) = saved.get("capture_mode").and_then(Value::as_str) {
        cfg.capture_mode = value.to_owned();
    }
    if let Some(value) = saved.get("max_image_dimension").and_then(Value::as_u64) {
        if let Ok(value) = u32::try_from(value) {
            cfg.max_image_dimension = value;
        }
    }
    cfg
}
...
impl ResizeRegistry {
    pub fn new() -> Self {
        let mut ratios = load_pid_state("resize").unwrap_or_default();
        ratios.retain(|_, ratio: &mut f64| ratio.is_finite() && *ratio > 0.0);
        Self {
    ...
    pub fn set_ratio(&self, pid: u32, ratio: f64) {
        ...
        bound_pid_map_before_insert(&mut ratios, pid);
        ratios.insert(pid, ratio);
        save_pid_state("resize", &ratios);
    }
...
impl ZoomRegistry {
    pub fn new() -> Self {
        let mut contexts = load_pid_state("zoom").unwrap_or_default();
        contexts.retain(|_, context: &mut ZoomContext| {
            context.origin_x.is_finite()
                && context.origin_y.is_finite()
                && context.scale_inv.is_finite()
                && context.scale_inv > 0.0
        });
        Self {
            inner: std::sync::Mutex::new(contexts),
        }
    }
    pub fn set(&self, pid: u32, ctx: ZoomContext) {
        let mut contexts = self.inner.lock().unwrap();
        bound_pid_map_before_insert(&mut contexts, pid);
        contexts.insert(pid, ctx);
        save_pid_state("zoom", &contexts);
    }
...
const PID_STATE_CAP: usize = 64;
const PID_STATE_MAX_BYTES: usize = 64 * 1024;

fn load_pid_state<T>(kind: &str) -> anyhow::Result<std::collections::HashMap<u32, T>>
where
    T: serde::de::DeserializeOwned,
{
    let Some(bytes) = crate::core::seat_context::read_state(kind, PID_STATE_MAX_BYTES)? else {
        return Ok(Default::default());
    };
    let state = serde_json::from_slice::<std::collections::HashMap<u32, T>>(&bytes)?;
    anyhow::ensure!(
        state.len() <= PID_STATE_CAP,
        "{kind} state exceeds its PID bound"
    );
    Ok(state)
}

fn save_pid_state<T>(kind: &str, state: &std::collections::HashMap<u32, T>)
where
    T: serde::Serialize,
{
...
fn bound_pid_map_before_insert<T>(state: &mut std::collections::HashMap<u32, T>, pid: u32) {
    if state.len() >= PID_STATE_CAP && !state.contains_key(&pid) {
        if let Some(oldest) = state.keys().copied().min() {
            state.remove(&oldest);
        }
    }
}
...
impl Tool for ListAppsTool {
    fn def(&self) -> &ToolDef {
        LIST_APPS_DEF.get_or_init(|| ToolDef {
            name: "list_apps".into(),
            description:
                "List visible installed Linux desktop applications exactly once, including \
                whether each application is currently running, \
                ...
                Running state comes from `/proc`. Applications come from XDG Desktop Entry \
                ...
                filtered with XDG override precedence. Service/helper processes that have no \
                visible desktop entry are not applications and are omitted. A `.desktop` file \
                whose launcher matches a running process (by basename) is returned once with \
                `running: true`.\n\n\
    ...
    async fn invoke(&self, _args: Value) -> ToolResult {
        let apps = tokio::task::spawn_blocking(|| -> Vec<serde_json::Value> {
            let procs = crate::platform::proc_fs::list_processes();
            let installed = crate::platform::installed_apps::list_installed_apps();
            let mut apps = visible_desktop_app_records(&procs, &installed);
            let windows = crate::platform::wayland::sway_ipc::list_public_windows()
                .unwrap_or_default();
            for app in &mut apps {
                let pid = app.get("pid").and_then(Value::as_u64).and_then(|pid| u32::try_from(pid).ok());
                let placements = windows.iter().filter(|(_, window)| Some(window.pid) == pid).map(|(window_id, window)| {
                    json!({
                        "window_id": window_id,
                        "title": window.title,
                        "workspace": window.workspace,
                        "workspace_index": match window.workspace.as_str() {
                            "Desktop" => Some(0),
                            "CUA" => Some(1),
                            name => name.strip_prefix("CUA").and_then(|suffix| suffix.parse::<u32>().ok()),
                        },
                        "output": window.output,
                    })
                }).collect::<Vec<_>>();
                app["windows"] = json!(placements);
            }
            apps
        })
        .await
...
fn visible_desktop_app_records(
    procs: &[crate::platform::proc_fs::ProcessInfo],
    installed: &[crate::platform::installed_apps::InstalledApp],
) -> Vec<serde_json::Value> {
    // Match running processes to installed apps by executable basename
    // (Exec=firefox %u → "firefox"; /usr/bin/firefox → "firefox").
    // Unmatched processes are system/session helpers, not launchable desktop
    // applications, and deliberately never enter this result.
    ...
    for (index, app) in installed.iter().enumerate() {
        ...
        if !basename.is_empty() {
            by_exe.entry(basename).or_default().push(index);
        }
    }
    let mut running = std::collections::HashMap::<usize, u32>::new();
    for process in procs {
        ...
        let candidates = by_exe.get(&basename).map(Vec::as_slice).unwrap_or(&[]);
        if let Some(index) = disambiguate_installed_match(candidates, installed, key_source) {
            running.entry(index).or_insert(process.pid);
        }
    }
    installed
        .iter()
        .enumerate()
        .map(|(index, app)| {
            let pid = running.get(&index).copied().unwrap_or(0);
            json!({
                "pid":         pid,
...
mod list_apps_tests {
    ...
    fn installed(name: &str, launch_path: &str) -> crate::platform::installed_apps::InstalledApp {
        crate::platform::installed_apps::InstalledApp {
            name: name.to_owned(),
            bundle_id: name.to_ascii_lowercase().replace(' ', "-"),
    ...
    #[test]
    fn visible_app_list_omits_unmatched_session_helpers() {
        let processes = vec![
            crate::platform::proc_fs::ProcessInfo {
                pid: 1,
            ...
            crate::platform::proc_fs::ProcessInfo {
                ...
                cmdline: "/usr/bin/browser".to_owned(),
            },
        ];
        let apps = vec![
            installed("Browser", "/usr/bin/browser"),
            installed("Files", "/usr/bin/files"),
        ];

        let records = visible_desktop_app_records(&processes, &apps);
        assert_eq!(records.len(), 2);
        assert_eq!(records[0]["name"], "Browser");
        assert_eq!(records[0]["pid"], 42);
        assert_eq!(records[0]["running"], true);
        assert_eq!(records[1]["name"], "Files");
        assert_eq!(records[1]["pid"], 0);
        assert!(records.iter().all(|record| record["name"] != "systemd"));
    }
}
...
impl Tool for ListWindowsTool {
    fn def(&self) -> &ToolDef {
        LIST_WINDOWS_DEF.get_or_init(|| ToolDef {
            ...
            description: "List every top-level window across Desktop and all numbered CUA workspaces. Each record includes compact workspace/output placement plus z_index (integer or null; \
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let (mut windows, geometry) = match tokio::task::spawn_blocking(move || {
            crate::platform::wayland::list_windows_dispatch_checked(filter_pid)
        })
        .await
        {
            Ok(Ok(result)) => result,
        ...
        if crate::platform::wayland::is_wayland() {
            crate::platform::wayland::remember_observed_window_origins(&windows);
        }
...
///
fn window_record_json(w: &crate::platform::x11::WindowInfo) -> Value {
    let mut record = json!({
        "window_id": w.xid,
        "pid": w.pid,
        "app_name": w.app_name,
    ...
    if let Ok(window) = crate::platform::wayland::sway_ipc::resolve_public_window(w.xid, w.pid) {
        record["workspace"] = json!(window.workspace);
        record["output"] = json!(window.output);
        record["workspace_index"] = json!(match window.workspace.as_str() {
            "Desktop" => Some(0),
            "CUA" => Some(1),
            name => name
                .strip_prefix("CUA")
...
mod list_windows_tests {
    ...
    fn record_has_bounds_and_flat_legacy_fields() {
        let w = crate::platform::x11::WindowInfo {
            xid: 42,
            pid: Some(1234),
...
impl Tool for GetWindowStateTool {
    fn def(&self) -> &ToolDef {
        GWS_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({"type":"object","required":["pid","window_id"],"properties":{
                ...
                "capture_mode": crate::core::capture_mode::capture_mode_schema(),
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<_> {
            let tree_result = Some(crate::platform::atspi::walk_tree_bounded(
                pid,
                xid,
            ...
            let screenshot = if should_capture {
                match crate::platform::wayland::screenshot_dispatch(xid) {
                    Ok(raw) => {
                        let orig_w = crate::platform::capture::png_dimensions_pub(&raw)
                            .map(|(w, _)| w)
                            .unwrap_or(0);
                        let png = crate::platform::capture::resize_png_if_needed(&raw, max_dim)?;
                        let (w, h) = crate::platform::capture::png_dimensions_pub(&png)?;
        ...
        match result {
            Ok(Ok((tree_opt, shot_opt, bounds))) => {
                ...
                if let Some(tr) = tree_opt {
                    ...
                    content.push(crate::core::protocol::Content::text(
                    ...
                    let snapshot_id = (!observation_only).then(|| {
                        crate::core::element_token::global()
                            .register_snapshot(pid as i32, xid as u32, count)
                    });
                    if let Some(snapshot_id) = snapshot_id {
                        let snapshot_handle = crate::core::element_token::token_for(snapshot_id, 0)
                            .trim_end_matches(":0")
                            .to_string();
                        crate::platform::atspi::register_snapshot_element_keys(
                            pid,
                            &snapshot_handle,
                            &tr.nodes,
                        );
                    ...
                    let elements: Vec<serde_json::Value> = tr
                        ...
                        .filter_map(|n| {
                            ...
                            if let Some(snapshot_id) = snapshot_id {
                                entry["element_token"] =
                                    json!(crate::core::element_token::token_for(snapshot_id, idx));
                            }
                            if n.in_web_content {
                        ...
                        .collect();
                    let elements = crate::core::element_query::project_elements_for_query(
                        elements,
                        query.as_deref(),
                        &tr.tree_markdown,
                    );
                    ...
                    if let Some(snapshot_id) = snapshot_id {
                        structured["snapshot_id"] =
                            json!(crate::core::element_token::token_for(snapshot_id, 0)
                                .trim_end_matches(":0")
                                .to_string());
                    }
                    ...
                    if !source_trusted {
                        ...
                        structured["degraded_reason"] = json!(
                            "x11_property_fallback_partial: AT-SPI was unavailable and \
                             Buzzard CUA only recovered window metadata. Treat it as \
                        ...
                        structured["degraded_reason"] = json!(
                            "atspi_tree_empty: the AT-SPI walk returned no actionable \
                             ...
                             toolkit-accessibility true`), the CLI is not on the \
                             guest desktop bus (DBUS_SESSION_BUS_ADDRESS unreachable — \
                             run `cua doctor`), or the window is a non-AX surface \
                ...
                if let Some((b64_opt, file_path, w, h, orig_w)) = shot_opt {
                    ...
                    if let Some(b64) = b64_opt {
                        content.push(crate::core::protocol::Content::image_png(b64));
                    }
...
fn route_launch_observation_to_cua(
    observation: DirectLaunchObservation,
    index: u32,
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if args.get("cdp_debugging_port").is_some() {
            return ToolResult::error(
                "Buzzard CUA has no CDP/browser tool; use `cua browser ...` for WildBuzzard automation",
            );
        }
        if launch_path_opt
            .as_deref()
        ...
        {
            return ToolResult::error(
                "Buzzard CUA does not enable browser remote debugging; use `cua browser ...` for WildBuzzard automation",
            );
        }
            ...
            move || -> anyhow::Result<(String, String, Option<DirectLaunchObservation>)> {
                ...
                if let Some(cmd) = command {
                    ...
                    launch
                        ...
                        .env("NO_AT_BRIDGE", "0")
                        // A launched desktop app outlives this one-shot CLI.
                        // Never let it inherit a caller's pipe/terminal and
                        // keep guest-exec, shell substitution, or an agent
                        // transport waiting until the GUI itself exits.
                        .stdin(Stdio::null())
                        .stdout(Stdio::null())
                        .stderr(Stdio::null());
        ...
        match result {
            Ok(Ok((message, name, observation))) => {
                if let Some(observation) = observation {
                    match route_launch_observation_to_cua(
                        observation,
                        crate::core::seat_context::current_index(),
                    ) {
                        Ok(observation) => direct_launch_result(message, name, observation),
                        Err(error) => ToolResult::error(format!(
                            "application launched but could not be routed into its numbered CUA workspace: {error:#}"
                        )),
                    }
                } else {
...
fn resolve_element_local_coords(
    ...
    let (bx, by, bw, bh) = crate::platform::atspi::get_element_bounds(pid, idx)?;
    ...
    if crate::platform::wayland::wayland_input_enabled() {
        let (window_x, window_y, window_width, window_height) =
            crate::platform::wayland::window_geometry(xid)
...
fn click_can_use_atspi_action(modifiers_empty: bool, button: u8, count: usize) -> bool {
    modifiers_empty && button == 1 && count == 1
}
...
/// `effect` tri-state. Native AT-SPI insertions are confirmed by an exact
/// Text-interface readback. Keystroke / XSendEvent / XTest / Wayland rungs
/// remain `"unverifiable"` (the caller confirms through a separate observation) and
...
/// Build the success `ToolResult` for an AT-SPI insert. Native widgets reach
/// this point only after exact Text-interface readback. Chromium embedders
/// remain unverifiable because their accessibility bridge can echo a mutation
...
fn unavailable_webkit_background(
    pid: u32,
    delivery: crate::platform::input::delivery::DeliveryMode,
) -> Option<ToolResult> {
    (!delivery.is_foreground()
        && is_webkitgtk_embedder(pid)
        && !crate::platform::input::real_pointer_input_available())
    .then(|| {
        crate::platform::input::delivery::background_unavailable_error(
            crate::platform::input::delivery::BackgroundUnavailable::WebKitSyntheticInput,
        )
    })
}
...
fn unavailable_webkit_keyboard_background(
    ...
    (!delivery.is_foreground() && is_webkitgtk_embedder(pid)).then(|| {
        crate::platform::input::delivery::background_unavailable_error(
            crate::platform::input::delivery::BackgroundUnavailable::FocusedInputOnly,
        )
    })
}
...
fn unavailable_wayland_focused_input_background(
    ...
    (crate::platform::wayland::wayland_input_enabled() && !delivery.is_foreground()).then(|| {
        crate::platform::input::delivery::background_unavailable_error(
            crate::platform::input::delivery::BackgroundUnavailable::FocusedInputOnly,
        )
    })
...
fn unavailable_chromium_background(
    ...
    if chromium_background_must_refuse(delivery.is_foreground(), is_chromium_embedder(pid)) {
        Some(
            crate::platform::input::delivery::background_unavailable_error(
                crate::platform::input::delivery::BackgroundUnavailable::ChromiumInput,
            ),
        )
    } else {
        None
...
fn chromium_background_must_refuse(foreground: bool, chromium: bool) -> bool {
    chromium && !foreground
}
...
fn x11_pixel_click_no_focus_steal(
    ...
    if crate::platform::input::real_pointer_input_available() {
        if let Ok((sx, sy)) = window_local_to_screen(xid, lx as f64, ly as f64) {
            match crate::platform::input::send_virtual_pointer_click(
                cursor_id,
                &crate::platform::input::VirtualPointerClick {
                    target_window: xid,
                    x: sx.round() as i32,
                    y: sy.round() as i32,
                    button,
    ...
    crate::platform::input::send_click(xid, lx, ly, count, button)
}
...
fn x11_pixel_click_no_focus_steal_modifiers(
    ...
    crate::platform::input::send_click_with_modifiers(xid, lx, ly, count, button, modifiers)
}
...
fn resolve_cursor_key(_: &Value) -> String {
    format!("cua{}", crate::core::seat_context::current_index())
}
...
fn mouse_hold_json(cursor_id: &str, hold: Option<&MouseHoldState>) -> Value {
    match hold {
        Some(hold) => json!({
            ...
            "pid": hold.pid,
            "window_id": hold.xid,
            "scope": if hold.desktop { "desktop" } else { "window" },
...
fn mouse_hold_success_json(cursor_id: &str, hold: Option<&MouseHoldState>, path: &str) -> Value {
    let mut result = mouse_hold_json(cursor_id, hold);
    result["path"] = json!(path);
    result["verified"] = json!(true);
    result["effect"] = json!("confirmed");
    result["evidence"] = json!([{"kind": "value_readback"}]);
    result
}

fn mouse_hold_path(desktop: bool) -> &'static str {
    match (crate::platform::wayland::is_wayland(), desktop) {
        (true, true) => "wayland_desktop",
        (true, false) => "wayland_window",
        (false, true) => "xtest_desktop",
        (false, false) => "x11_window",
    }
}
...
fn held_target_mismatch(
    ...
    if let Some(scope) = args.opt_str("scope") {
        let held_scope = if hold.desktop { "desktop" } else { "window" };
        if scope != held_scope {
            return Some(
                ToolResult::error(format!(
                    "Cursor '{cursor_id}' is holding a button in {held_scope} scope, not {scope} scope."
                ))
...
fn is_terminal_process(pid: u32) -> bool {
    // Canonical list lives in `crate::platform::terminal::TERMINAL_PROCESS_NAMES`
    // — keeps the additive contract centralised. Adding a new terminal
    // here means appending one string in `crate::platform::terminal`.
    match process_name(pid).as_deref() {
        Some(name) => crate::platform::terminal::is_terminal_process_name(name),
        None => false,
    }
}
...
fn terminal_tty_for_window(pid: u32, xid: u64) -> Option<PathBuf> {
    ...
    let mut windows = crate::platform::x11::list_windows(Some(pid));
...
/// `crate::platform::tty`). Returns `Ok(false)` when the target isn't a terminal we can
...
fn inject_terminal_input(pid: u32, xid: u64, text: &str) -> anyhow::Result<bool> {
    ...
    crate::platform::tty::inject_via_master(pid, ptn, text)
}
...
impl Tool for ClickTool {
    fn def(&self) -> &ToolDef {
        CLICK_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({
                ...
                // in crate::core::tool_schema.
                "type":"object","required":[],"properties":{
                    "pid":{"type":"integer"},
                    "window_id":{"type":"integer"},
                    "x":{"type":"number"},
                    "y":{"type":"number"},
                    "element_index": crate::core::tool_schema::element_index_schema(),
                    "element_token": crate::core::tool_schema::element_token_schema(),
                    "snapshot_id": crate::core::tool_schema::snapshot_id_schema(),
                    ...
                    "delivery_mode": crate::platform::input::delivery::delivery_mode_schema()
                },"additionalProperties":false
            }),
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        // ── Window-less screen-absolute branch ──────────────────────────────
        ...
        // This branch validates the explicit full-output action form.
        ...
        if has_xy && !has_pid && !has_window_id {
            ...
            let r = tokio::task::spawn_blocking(move || {
                if crate::platform::wayland::wayland_input_enabled() {
                    ...
                    crate::platform::wayland::click_desktop(sx, sy, n as u32, button)
                } else {
                    let modifier_refs: Vec<&str> = modifiers.iter().map(String::as_str).collect();
                    crate::platform::input::send_click_xtest_desktop_with_modifiers(
                        sx,
                        sy,
                        button,
                        n,
            ...
            return match r {
                Ok(Ok(())) => {
                    let path = if crate::platform::wayland::wayland_input_enabled() {
                        "wayland_desktop"
                    } else {
                        "xtest_desktop"
                    };
                    let observation =
                        observe_guest_output_change_masked(visual_before, path, &cursor_masks)
                            .await;
                    ToolResult::text(format!(
                        "Sent screen-absolute click at ({sx},{sy}) and observed the guest output."
                    ))
        ...
        let delivery = crate::platform::input::delivery::DeliveryMode::from_args(&args);
        ...
        let snapshot_id_arg = args.opt_str("snapshot_id");
        let window_id_arg = args.opt_u64("window_id");
        let element_index_arg = args.opt_u64("element_index").map(|v| v as usize);
        let resolved = match crate::core::element_token::resolve_element_args(
            pid as i32,
            element_index_arg,
            element_token_arg.as_deref(),
            snapshot_id_arg.as_deref(),
        ...
        let elem_idx_resolved: Option<usize> = match &resolved {
            crate::core::element_token::ResolvedElement::Element { element_index, .. } => {
                Some(*element_index)
            }
            crate::core::element_token::ResolvedElement::None => None,
        };
        let window_id_resolved: Option<u64> = match &resolved {
            crate::core::element_token::ResolvedElement::Element { window_id, .. } => {
                window_id_arg.or_else(|| window_id.map(|v| v as u64))
            }
            crate::core::element_token::ResolvedElement::None => window_id_arg,
        };

        if let Some(idx) = elem_idx_resolved {
            let snapshot_handle = element_token_arg
                .as_deref()
                .and_then(|token| {
                    token
                        .split_once(':')
                        .map(|(snapshot, _)| snapshot.to_owned())
                })
                .or(snapshot_id_arg);
            let stable_element_key = snapshot_handle.as_deref().and_then(|snapshot| {
                crate::platform::atspi::snapshot_element_key(pid, snapshot, idx)
            });
            if snapshot_handle.is_some() && stable_element_key.is_none() {
                return ToolResult::error(
                ...
                .with_structured(json!({
                    ...
                    "refusal": {
                        ...
                        "message": "the bounded daemonless AT-SPI snapshot is no longer available"
                    }
                }));
            }
            ...
            // AT-SPI activation represents one ordinary primary-button
            // activation. A right-click or double-click must retain its
            // requested button/count and therefore uses the physical route
            // through the element's observed output coordinates below.
            if click_can_use_atspi_action(modifiers.is_empty(), button, count) {
                let ax_result = tokio::task::spawn_blocking(move || {
                    if let Some(key) = stable_element_key {
                        crate::platform::atspi::perform_action_by_key(pid, key)
                    } else {
                        crate::platform::atspi::perform_action(pid, idx)
                    }
                })
                .await;
                if let Ok(Ok((_action, suspected_noop))) = ax_result {
                    let mut structured =
                        observe_guest_output_change_masked(visual_before, "ax", &cursor_masks)
                            .await;
                    if suspected_noop
                        && structured.get("effect").and_then(Value::as_str) != Some("confirmed")
                    {
            ...
            // The AX route was unavailable or cannot express the requested
            // gesture. Fall back to compositor-global Wayland input or a
            // target-addressed X11 event.
            let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                ...
                if crate::platform::wayland::wayland_input_enabled() {
                    ...
                    return crate::platform::wayland::click_desktop(
                        sx.round() as i32,
                        sy.round() as i32,
                        count as u32,
                        button,
                ...
                if delivery.is_foreground() && !crate::platform::wayland::wayland_input_enabled() {
                    crate::platform::input::with_x11_foreground(xid2, 80, || {
                        crate::platform::input::send_click_xtest_desktop_with_modifiers(
                            sx.round() as i32,
                            sy.round() as i32,
                            button,
                    ...
                    crate::platform::input::send_click_with_modifiers(
                        xid2,
                        lx as i32,
                        ly as i32,
                        count,
            ...
            return match result {
                Ok(Ok(())) => {
                    let structured = observe_guest_output_change_masked(
                        visual_before,
                        "x11_pixel",
                        &cursor_masks,
                    )
        ...
        let (visual_before, cursor_masks) = match glide_target {
            ...
            None => (capture_guest_output().await, Vec::new()),
        };
        ...
        // Normal delivery focuses the target only on this numbered CUA seat.
        // Explicit background remains a compatibility request and must prove
        // target-addressed delivery.
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<&'static str> {
            if crate::platform::wayland::wayland_input_enabled() {
                ...
                if !delivery.is_foreground() && button == 1 && count == 1 {
                    if let Ok(Some(_)) = crate::platform::atspi::perform_action_at_screen_point(
            ...
            let inject = |fg: bool| -> anyhow::Result<&'static str> {
                if !fg && button == 1 && count == 1 && modifiers_for_task.is_empty() {
                    if let Ok(Some(_)) =
                        crate::platform::atspi::perform_action_at_point(pid, xi, yi)
                    {
                        return Ok("x11_atspi");
                    }
                }
            ...
            if delivery.is_foreground() {
                crate::platform::input::with_x11_foreground(xid, 80, || inject(true))
            } else {
                inject(false)
            }
        })
        ...
        match result {
            ...
            Ok(Ok(path)) => {
                let observation =
                    observe_guest_output_change_masked(visual_before, path, &cursor_masks).await;
                ToolResult::text(format!(
                    "Clicked at ({x:.1}, {y:.1}) × {count} \
                     (delivery_mode={mode_label}) and observed the guest output."
                ))
                .with_structured(observation)
            }
...
/// `window_id` is `u64` (the X11 XID / window id). The numbered `cuaN`
/// executable selects the cursor and virtual seat.
...
impl Tool for TypeTextTool {
        ...
        TYPE_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({
                "type":"object","required":["text"],"properties":{
                    "pid":{"type":"integer"},
                    "window_id":{"type":"integer"},
                    "text":{"type":"string","maxLength":crate::contract::MAX_TYPE_TEXT_CHARS},
                    "element_index": crate::core::tool_schema::element_index_schema(),
                    "element_token": crate::core::tool_schema::element_token_schema(),
                    "snapshot_id": crate::core::tool_schema::snapshot_id_schema(),
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if args.opt_str("scope").as_deref() == Some("desktop") && args.get("pid").is_none() {
            ...
            let text = crate::core::text_sanitize::strip_trailing_agent_protocol_tags(&input.text)
                .into_owned();
            ...
            let result = tokio::task::spawn_blocking(move || {
                if wayland {
                    crate::platform::wayland::type_text_focused(&keyboard_admission, &text)
                } else {
                    crate::platform::input::send_type_text_xtest(&text)
                }
            })
            .await;
            return match result {
                Ok(Ok(())) => {
                    let observation = observe_guest_output_change(visual_before, path).await;
                    ToolResult::text(
                        "Typed text into the focused desktop application and observed the guest output.",
                    )
                    .with_structured(observation)
                }
        ...
        // crate::core::text_sanitize docs for rationale.
        let text =
            crate::core::text_sanitize::strip_trailing_agent_protocol_tags(&text_raw).into_owned();
        ...
        let (resolved_elem_idx, resolved_window_id) = match &resolved {
            crate::core::element_token::ResolvedElement::Element {
                element_index,
                window_id,
                ..
            } => (Some(*element_index), window_id.map(|v| v as u64)),
            crate::core::element_token::ResolvedElement::None => (None, None),
        };
        ...
        if resolved_elem_idx.is_some() && !is_chromium_embedder(pid) && !is_webkitgtk_embedder(pid)
        {
            let idx = resolved_elem_idx.expect("checked above");
            let text_at = text.clone();
            let targeted = tokio::task::spawn_blocking(move || {
                crate::platform::atspi::type_into_editable_at(pid, idx, &text_at)
            })
            .await;
            match targeted {
                Ok(Ok(())) => {
                ...
                Ok(Err(error))
                    if crate::platform::atspi::is_unverified_editable_mutation(&error) =>
                {
            ...
            && (is_chromium_embedder(pid) || is_webkitgtk_embedder(pid))
        {
            if let Some(idx) = resolved_elem_idx {
                ...
                let keyboard_admission = keyboard_admission.clone();
                let result = tokio::task::spawn_blocking(move || {
                    crate::platform::wayland::type_text(&keyboard_admission, xid, &text_w)
                })
                .await;
                return match result {
                    Ok(Ok(())) => {
            ...
            match targeted {
                ...
                Ok(Err(_)) | Err(_)
                    if !delivery.is_foreground()
                        && crate::platform::wayland::wayland_input_enabled() =>
                {
                    return crate::platform::input::delivery::background_unavailable_error(
                        crate::platform::input::delivery::BackgroundUnavailable::FocusedInputOnly,
                    );
                }
                _ => {}
        ...
        if crate::platform::wayland::wayland_input_enabled() {
            ...
            return match result {
                Ok(Ok(())) => {
                    let observation =
                        observe_typed_text(visual_before, &cursor_masks, "key_events", text_len)
                            .await;
                    ToolResult::text(format!(
                        "Typed {text_len} character(s) (via Wayland virtual-keyboard)."
                    ))
        ...
        let wm_class_is_terminal =
            tokio::task::spawn_blocking(move || crate::platform::terminal::is_terminal_window(xid))
                .await
                .unwrap_or(false);
        if pid_is_terminal || wm_class_is_terminal {
            ...
            let result = tokio::task::spawn_blocking(move || -> anyhow::Result<&'static str> {
                ...
                if foreground {
                    crate::platform::input::with_x11_foreground(xid, 80, || {
                        crate::platform::input::send_type_text_xtest(&text_t)
                    })?;
                    Ok("key_events_fg")
                } else {
        ...
        if delivery.is_foreground() && (is_chromium_embedder(pid) || is_webkitgtk_embedder(pid)) {
            ...
            let result = tokio::task::spawn_blocking(move || {
                crate::platform::input::with_x11_foreground(xid, 80, || {
                    crate::platform::input::send_type_text_xtest(&text_f)
                })
            })
            .await;
            return match result {
        ...
        let focus_kind = tokio::task::spawn_blocking(move || {
            crate::platform::atspi::focused_is_editable(pid)
                .ok()
                .flatten()
        })
        .await
        ...
        let atspi_result = tokio::task::spawn_blocking(move || {
            crate::platform::atspi::type_into_editable(pid, &text_clone)
        })
        .await;
        ...
        match atspi_result {
            Ok(Ok(())) => {
                ...
                // Native widgets have already been read back exactly. Electron/
                // Chromium can still echo this write without the renderer observing
                // it, so the confirm is suppressed there (mirrors macOS).
                return type_text_ax_result(pid, text_len, "via AT-SPI");
            }
            Ok(Err(error)) if crate::platform::atspi::is_unverified_editable_mutation(&error) => {
                return ToolResult::error(error.to_string());
            }
            _ => {
        ...
        let qt5_result = tokio::task::spawn_blocking(move || {
            ...
            crate::platform::input::send_focus_in(xid)?;
            std::thread::sleep(std::time::Duration::from_millis(100));

            // Try AT-SPI again now that widgets should be exposed
            let result = crate::platform::atspi::type_into_editable(pid, &text_clone2);

            // Restore state with FocusOut
            crate::platform::input::send_focus_out(xid)?;
        ...
        // Normal delivery focuses the target only on this numbered CUA seat.
        // Explicit background may use AT-SPI or target-addressed Xwayland
        // delivery when it can be confirmed.
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<&'static str> {
            ...
            match crate::platform::atspi::insert_text(pid, &text) {
                Ok(true) => return Ok("ax"),
                Ok(false) => {}
                Err(error) if crate::platform::atspi::is_unverified_editable_mutation(&error) => {
                    return Err(error);
                }
                Err(_) => {}
            }
            ...
            if crate::platform::input::inject_tk_send(&text).unwrap_or(false) {
                return Ok("key_events");
            }
            if delivery.is_foreground() {
                crate::platform::input::with_x11_foreground(xid, 80, || {
        ...
        match result {
            // Native AT-SPI insertions were read back exactly. Chromium keeps
            // its stricter renderer-echo handling.
...
impl Tool for PressKeyTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if args.opt_str("scope").as_deref() == Some("desktop") && args.get("pid").is_none() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                if wayland && modifiers.is_empty() {
                    crate::platform::wayland::press_key_focused(&keyboard_admission, &key)
                } else if wayland {
                    let mut keys = modifiers;
                    keys.push(key);
                    crate::platform::wayland::hotkey_focused(&keyboard_admission, &keys)
                } else {
                    let refs: Vec<&str> = modifiers.iter().map(String::as_str).collect();
                    crate::platform::input::send_key_xtest(&key, &refs)
                }
            })
            .await;
            return match result {
                Ok(Ok(())) => {
                    let observation = observe_guest_output_change(visual_before, path).await;
                    ToolResult::text(format!(
                        "Pressed desktop key '{display}' and observed the guest output."
                    ))
                    .with_structured(observation)
                }
                Ok(Err(error)) => ToolResult::error(error.to_string()),
        ...
        if let Some(element_index) = element_index_arg {
            let focused = tokio::task::spawn_blocking(move || {
                crate::platform::atspi::focus_element(pid, element_index)
            })
            .await;
            match focused {
                Ok(Ok(true)) => {}
        ...
        if crate::platform::wayland::wayland_input_enabled() {
            ...
            let result = match press_key_chord(&mods, &key) {
                None => {
                    ...
                    tokio::task::spawn_blocking(move || {
                        crate::platform::wayland::press_key(&keyboard_admission, xid, &key_w)
                    })
                    .await
                }
                Some(chord) => {
                    let keyboard_admission = keyboard_admission.clone();
                    tokio::task::spawn_blocking(move || {
                        crate::platform::wayland::hotkey(&keyboard_admission, xid, &chord)
                    })
                    .await
                }
            };
            return match result {
                Ok(Ok(())) => {
                    let observation = observe_guest_output_change_masked(
                        visual_before,
                        "key_events",
                        &cursor_masks,
                    )
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            ...
            if deliver_fg {
                return crate::platform::input::with_x11_foreground(xid, 80, || {
                    crate::platform::input::send_key_xtest(&key_for_task, &m)
                });
            }
            if let Some((x, y)) = px_target {
                crate::platform::input::send_key_at(xid, x, y, &key_for_task, &m)
            } else {
                crate::platform::input::send_key(xid, &key_for_task, &m)
            }
        })
        .await;
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if args.opt_str("scope").as_deref() == Some("desktop") && args.get("pid").is_none() {
            ...
            return match result {
                Ok(Ok(())) => {
                    let observation = observe_guest_output_change(visual_before, path).await;
                    ToolResult::text(format!(
                        "Pressed desktop hotkey {display} and observed the guest output."
                    ))
                    .with_structured(observation)
                }
                Ok(Err(error)) => ToolResult::error(error.to_string()),
            ...
            && is_chromium_embedder(pid)
        {
            if let Some(element_index) = resolved_element_index {
                ...
                if let Err(error) =
                    focus_by_pixel(&self.state, pid, Some(xid), x, y, true, false).await
                {
                    return error;
                }
            }
        ...
        let deliver_fg = delivery.is_foreground();
        let (visual_before, cursor_masks) = capture_guest_output_with_current_pointer_mask().await;
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            if crate::platform::wayland::wayland_input_enabled() {
                ...
                return crate::platform::wayland::hotkey(&keyboard_admission, xid, &combo);
            }
            let m: Vec<&str> = mods.iter().map(String::as_str).collect();
            ...
            if deliver_fg {
                return crate::platform::input::with_x11_foreground(xid, 80, || {
                    crate::platform::input::send_key_xtest(&key, &m)
                });
            }
            if let Some((x, y)) = px_target {
                crate::platform::input::send_key_at(xid, x, y, &key, &m)
            } else {
                crate::platform::input::send_key(xid, &key, &m)
            }
        })
        .await;
        let mode_label = if deliver_fg {
        ...
        match result {
            Ok(Ok(())) => {
                let mut observation =
                    observe_guest_output_change_masked(visual_before, "key_events", &cursor_masks)
                        .await;
                observation["delivery_mode"] = json!(mode_label);
                ToolResult::text(format!(
...
impl Tool for SetValueTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let idx = match resolved {
            ...
            crate::core::element_token::ResolvedElement::None => return ToolResult::error(
...
impl Tool for ScrollTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
                ...
                Ok(Ok(())) => {
                    let observation =
                        observe_guest_output_change_masked(visual_before, path, &cursor_masks)
                            .await;
                    ToolResult::text(format!(
                        "Scrolled desktop {display} × {amount} and observed the guest output."
                    ))
                    .with_structured(observation)
                }
                Ok(Err(error)) => ToolResult::error(error.to_string()),
        ...
        let xid_opt: Option<u64> = match &resolved {
            crate::core::element_token::ResolvedElement::Element { window_id, .. } => window_id
                .map(|v| v as u64)
                .or_else(|| args.opt_u64("window_id")),
            crate::core::element_token::ResolvedElement::None => args.opt_u64("window_id"),
        };
        ...
        if let crate::core::element_token::ResolvedElement::Element { element_index, .. } =
            &resolved
        {
            ...
            if !(crate::platform::wayland::wayland_input_enabled() && is_webkitgtk_embedder(pid)) {
                let direction_for_ax = direction.clone();
                let ax_result = tokio::task::spawn_blocking(move || {
                    crate::platform::atspi::scroll_element(pid, idx, &direction_for_ax, amount)
                })
                .await;
        ...
        if crate::platform::wayland::wayland_input_enabled() {
            ...
            let local_point = match (pixel_target, &resolved) {
                ...
                (None, crate::core::element_token::ResolvedElement::None) => None,
            };
        ...
        let element_point = match &resolved {
            ...
            crate::core::element_token::ResolvedElement::None => pixel_target.map(|local| {
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            if crate::platform::wayland::wayland_input_enabled() {
                return crate::platform::wayland::scroll(xid, &direction_for_wayland, amount_u32);
            }
            ...
            let x11_scroll = || -> anyhow::Result<()> {
                ...
                if crate::platform::input::real_pointer_input_available() {
                    ...
                    if let Some((cx, cy)) = pointer_point {
                        ...
                        match crate::platform::input::send_virtual_pointer_scroll(
                            &cursor_id_for_task,
                            &crate::platform::input::VirtualPointerScroll {
                                x: cx,
                                y: cy,
                                horizontal,
                                ticks,
                ...
                crate::platform::input::send_click(
                    xid,
                    local_x as i32,
                    local_y as i32,
                    amount,
            ...
            if delivery.is_foreground() {
                ...
                crate::platform::input::with_x11_foreground(xid, 80, || {
                    crate::platform::input::send_click_xtest_desktop(
                        point.0 as i32,
                        point.1 as i32,
                        button,
                        amount,
...
impl Tool for DoubleClickTool {
    fn def(&self) -> &ToolDef {
        DCLICK_DEF.get_or_init(|| ToolDef {
            ...
            description: "Double-click through the same desktop/window/AT-SPI route as click, \
                with two left-button activations and the same post-action evidence. Provide \
                desktop x/y, window_id + x/y, or an AT-SPI element token/index. After a zoom \
                call, pass from_zoom=true to translate zoom-image coordinates.".into(),
            input_schema: json!({"type":"object","required":[],"properties":{
                "pid":{"type":"integer"},
                "window_id":{"type":"integer"},
                "x":{"type":"number"},
                "y":{"type":"number"},
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        // Tool arguments are schema-validated objects. Reuse ClickTool for
        // every supported target form so double-click cannot drift onto an
        // unverified legacy delivery path.
        if args.is_object() {
            let mut click_args = args;
            click_args["count"] = json!(2);
            click_args["button"] = json!("left");
            return ClickTool {
        ...
        let window_id_resolved: Option<u64> = match &resolved {
            crate::core::element_token::ResolvedElement::Element { window_id, .. } => args
                .opt_u64("window_id")
            ...
            return match result {
                Ok(Ok((xid, lx, ly))) => {
                    ...
                    let wayland_point = crate::platform::wayland::wayland_input_enabled()
                        .then(|| crate::platform::wayland::window_local_to_output(xid, lxi, lyi));
                    let cursor_id_for_task = cursor_id.clone();
                    let click_result = tokio::task::spawn_blocking(move || {
                        if crate::platform::wayland::wayland_input_enabled() {
                            let (output_x, output_y) = wayland_point.unwrap_or((lxi, lyi));
                            return crate::platform::wayland::click(xid, output_x, output_y, 2, 1);
                        }
                        if delivery.is_foreground() {
                            return crate::platform::input::with_x11_foreground(xid, 80, || {
...
impl Tool for RightClickTool {
    fn def(&self) -> &ToolDef {
        RCLICK_DEF.get_or_init(|| ToolDef {
            ...
            description: "Right-click through the same desktop/window/AT-SPI route as click, \
                with the same post-action evidence. Provide desktop x/y, window_id + x/y, or \
                an AT-SPI element token/index. After a zoom call, pass from_zoom=true to \
                translate zoom-image coordinates.".into(),
            input_schema: json!({"type":"object","required":[],"properties":{
                "pid":{"type":"integer"},
                "window_id":{"type":"integer"},
                "x":{"type":"number"},
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        // Tool arguments are schema-validated objects. Reuse ClickTool for
        // every supported target form so right-click cannot drift onto an
        // unverified legacy delivery path.
        if args.is_object() {
            let mut click_args = args;
            click_args["count"] = json!(1);
            click_args["button"] = json!("right");
            return ClickTool {
                state: self.state.clone(),
            }
            .invoke(click_args)
            ...
            return match result {
                Ok(Ok((xid, lx, ly))) => {
                    ...
                    let click_result = tokio::task::spawn_blocking(move || {
                        if crate::platform::wayland::wayland_input_enabled() {
                            let (output_x, output_y) = wayland_point.unwrap_or((lxi, lyi));
                            return crate::platform::wayland::click(xid, output_x, output_y, 1, 3);
                        }
                        if delivery.is_foreground() {
                            return crate::platform::input::with_x11_foreground(xid, 80, || {
                                let (sx, sy) = window_local_to_screen(xid, lxi as f64, lyi as f64)?;
...
impl Tool for DragTool {
    fn def(&self) -> &ToolDef {
        DRAG_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({"type":"object","required":["from_x","from_y","to_x","to_y"],"properties":{
                ...
                "modifier": crate::core::tool_schema::modifier_schema(),
                "button": crate::core::tool_schema::button_schema(),
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
            ...
            && args.get("window_id").is_none()
        {
            ...
            let (visual_before, mut cursor_masks) =
                capture_guest_output_after_pointer_at(from_x.round() as i32, from_y.round() as i32)
                    .await;
            let drag_end = (to_x.round() as i32, to_y.round() as i32);
            if !cursor_masks.contains(&drag_end) {
                cursor_masks.push(drag_end);
            }
            let result = tokio::task::spawn_blocking(move || {
                if wayland {
                    crate::platform::wayland::drag_desktop(
                        &keyboard_admission,
                        from_x.round() as i32,
                        from_y.round() as i32,
                        to_x.round() as i32,
                    ...
                    crate::platform::input::send_drag_xtest_desktop(
                        from_x.round() as i32,
                        from_y.round() as i32,
                        to_x.round() as i32,
                        to_y.round() as i32,
            ...
            return match result {
                Ok(Ok(())) => {
                    let observation =
                        observe_guest_output_change_masked(visual_before, path, &cursor_masks)
                            .await;
                    ToolResult::text("Dragged on the desktop and observed the guest output.")
                        .with_structured(observation)
                }
                Ok(Err(error)) => ToolResult::error(error.to_string()),
                Err(error) => ToolResult::error(format!("Task error: {error}")),
        ...
        if result.is_ok() {
            ...
            for i in 1..=steps {
                ...
                let motion_result = tokio::task::spawn_blocking(move || {
                    crate::platform::input::send_motion(
                        xid,
                        ix.round() as i32,
                        iy.round() as i32,
                        Some(button),
        ...
        let release_result = tokio::task::spawn_blocking(move || {
            crate::platform::input::send_button_up(
                xid,
                to_x.round() as i32,
                to_y.round() as i32,
                button,
...
fn held_pointer_batch_required() -> Option<ToolResult> {
    (std::env::var_os("BUZZARDOS_CUA_BATCH").as_deref() != Some(std::ffi::OsStr::new("1")))
        .then(|| {
            ToolResult::error(
                "A held pointer cannot survive process exit. Run mouse_button_down, mouse_drag, and mouse_button_up in one daemonless `cua batch` invocation, or use the standalone drag tool.",
            )
            .with_structured(json!({
                "status":"refused",
                "refusal":{"code":"batch_required","recovery":"use cua batch or drag"}
            }))
        })
}
...
impl Tool for MouseButtonDownTool {
    fn def(&self) -> &ToolDef {
        MDOWN_DEF.get_or_init(|| ToolDef {
            ...
            description: "Press and hold a mouse button at (x,y). Window scope uses window-local \
                screenshot coordinates; desktop scope uses canonical guest-output coordinates. \
                Does not release the button; pair with mouse_drag / mouse_button_up. \
                The three calls must be in one daemonless cua batch invocation. \
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        if let Some(refusal) = held_pointer_batch_required() {
            return refusal;
        }
        let cursor_id = resolve_cursor_key(&args);
        ...
        if desktop && (args.get("pid").is_some() || args.get("window_id").is_some()) {
            return ToolResult::error(
                "mouse_button_down desktop scope does not accept pid/window_id.",
            );
        }
        let pid = if desktop {
            0
        } else {
            match args.require_u32("pid") {
                Ok(v) => v,
                Err(e) => return e,
            }
        };
        let xid = if desktop {
            ...
            match args.opt_u64("window_id") {
                Some(v) => v,
                None => return ToolResult::error("window_id is required in window scope."),
            }
        };
        ...
        let mut x = args.f64_or("x", 0.0);
        let mut y = args.f64_or("y", 0.0);
        if desktop && args.bool_or("from_zoom", false) {
            return ToolResult::error("from_zoom is unavailable in desktop scope.");
        }
        ...
        let output_point = if desktop {
            (xi, yi)
        } else if crate::platform::wayland::wayland_input_enabled() {
            crate::platform::wayland::window_local_to_output(xid, xi, yi)
        } else {
            (xi, yi)
        };
        ...
        let result = if crate::platform::wayland::is_wayland() {
            let cid = cursor_id.clone();
            if desktop {
                tokio::task::spawn_blocking(move || {
                    crate::platform::wayland::persistent_vptr::press_desktop(
                        &cid,
                        output_point.0,
                        output_point.1,
                        button,
                ...
                tokio::task::spawn_blocking(move || {
                    crate::platform::wayland::persistent_vptr::press(
                        &cid,
                        xid,
                        output_point.0,
                        output_point.1,
            ...
            tokio::task::spawn_blocking(move || {
                crate::platform::input::send_button_down_xtest_desktop(xi, yi, button)
            })
            .await
        } else {
            tokio::task::spawn_blocking(move || {
                crate::platform::input::send_button_down(xid, xi, yi, button)
            })
            .await
        };
        match result {
            Ok(Ok(())) => {
                ...
                .with_structured(mouse_hold_success_json(
                    &cursor_id,
                    Some(&hold),
                    mouse_hold_path(desktop),
                ))
            }
...
impl Tool for MouseDragTool {
    fn def(&self) -> &ToolDef {
        MDRAG_DEF.get_or_init(|| ToolDef {
            ...
            description: "Move a previously-held mouse button to a new point in the same window or \
                canonical desktop coordinate scope selected by mouse_button_down. \
                Requires an active mouse_button_down state; does not release the button. \
                Must run in the same daemonless cua batch as down/up. Returns the updated \
                held-button state.".into(),
            input_schema: json!({"type":"object","required":["x","y"],"properties":{
                "pid":{"type":"integer"},
                "window_id":{"type":"integer"},
                "x":{"type":"number"},
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if hold.desktop && args.bool_or("from_zoom", false) {
            return ToolResult::error("from_zoom is unavailable in desktop scope.")
                .with_structured(mouse_hold_json(&cursor_id, Some(&hold)));
        }
        if args.bool_or("from_zoom", false) {
        ...
        } else if !hold.desktop {
            if let Some(ratio) = self.state.resize_registry.ratio(hold.pid) {
                to_x *= ratio;
                to_y *= ratio;
            }
        }
        ...
        for i in 1..=steps {
            ...
            let move_result = if is_wl {
                let (motion_x, motion_y) = if desktop {
                ...
                tokio::task::spawn_blocking(move || {
                    crate::platform::wayland::persistent_vptr::move_to(
                        &cid_inner, motion_x, motion_y,
                    )
                })
                .await
            } else if desktop {
                tokio::task::spawn_blocking(move || {
                    crate::platform::input::send_move_xtest_desktop(
                        ix.round() as i32,
                        iy.round() as i32,
                    )
                })
...
impl Tool for MouseButtonUpTool {
    fn def(&self) -> &ToolDef {
        MUP_DEF.get_or_init(|| ToolDef {
            ...
            description: "Release a previously-held mouse button in the same daemonless cua batch. \
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let result = if crate::platform::wayland::is_wayland() {
            let cid = cursor_id.clone();
            tokio::task::spawn_blocking(move || {
                crate::platform::wayland::persistent_vptr::release(&cid, button)
            })
            .await
        } else if hold.desktop {
            tokio::task::spawn_blocking(move || {
                crate::platform::input::send_button_up_xtest_desktop(xi, yi, button)
            })
            .await
        } else {
            tokio::task::spawn_blocking(move || {
                crate::platform::input::send_button_up(xid, xi, yi, button)
            })
            .await
        };
        match result {
...
impl Tool for GetScreenSizeTool {
    fn def(&self) -> &ToolDef {
        GSS_DEF.get_or_init(|| ToolDef {
            ...
            description: "Return the main display's canonical physical-pixel extent plus its \
                separately reported guest logical mode and desktop scale. Agents click in the \
                physical coordinate space. Requires no TCC permissions."
                .into(),
            input_schema: json!({"type":"object","properties":{},"additionalProperties":false}),
            read_only: true,
            destructive: false,
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let result = tokio::task::spawn_blocking(|| {
            if crate::platform::wayland::is_wayland() {
                let geometry = crate::platform::wayland::canonical_output_metadata_checked(1, 1)?;
                if (geometry.physical_width, geometry.physical_height) != (1, 1) {
                    return Ok::<crate::platform::wayland::CanonicalOutputMetadata, anyhow::Error>(
                        geometry,
                    );
                }
            }
            ...
            Ok::<crate::platform::wayland::CanonicalOutputMetadata, anyhow::Error>(
                crate::platform::wayland::canonical_output_metadata(w, h),
            )
        })
        .await;
        match result {
...
fn x11_screen_size() -> anyhow::Result<(u32, u32)> {
    ...
    use x11rb::rust_connection::RustConnection;
    let (conn, screen_num) = RustConnection::connect(None)
        .map_err(|e| anyhow::anyhow!("{e}{}", crate::platform::no_display_hint()))?;
    ...
    if w == 0 || h == 0 {
        anyhow::bail!(
            "X11 connected but reports a 0x0 root screen — no usable \
             display geometry.{}",
            crate::platform::no_display_hint()
        );
    }
    Ok((w, h))
}
...
impl Tool for GetDesktopStateTool {
    fn def(&self) -> &ToolDef {
        GDS_DEF.get_or_init(|| ToolDef {
            ...
            description: "Full-output screenshot of the caller's numbered CUA workspace in native \
                physical pixels. Screenshot pixels, AT-SPI geometry, and absolute input coordinates \
                agree. No AT-SPI walk.".into(),
            input_schema: json!({"type":"object","properties":{
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<_> {
            ...
            let (png, geometry) = if crate::platform::wayland::is_wayland() {
                crate::platform::wayland::screenshot_display_dispatch_with_metadata()?
            } else {
                let png = crate::platform::capture::screenshot_display_bytes()?;
                let (width, height) = crate::platform::capture::png_dimensions_pub(&png)?;
                (
                    png,
                    crate::platform::wayland::canonical_output_metadata(width, height),
                )
            };
            let (shot_w, shot_h) = crate::platform::capture::png_dimensions_pub(&png)?;
            ...
            let (screen_w, screen_h) = if crate::platform::wayland::is_wayland() {
        ...
        match result {
            Ok(Ok((b64_opt, shot_w, shot_h, screen_w, screen_h, geometry, written))) => {
                ...
                let mut structured = json!({
                    "platform": "linux",
                    "display": std::env::var(crate::core::seat_context::CUA_OUTPUT_ENV)
                        .unwrap_or_else(|_| "caller-output".to_owned()),
                ...
                if let Some(path) = written {
                    ...
                    content.push(crate::core::protocol::Content::text(format!(
...
impl Tool for GetCursorPositionTool {
    fn def(&self) -> &ToolDef {
        GCP_DEF.get_or_init(|| ToolDef {
            ...
            description:
                "Return the current mouse cursor position in canonical physical pixels (origin top-left)."
                    .into(),
            input_schema: json!({"type":"object","properties":{},"additionalProperties":false}),
            read_only: true,
            destructive: false,
...
impl Tool for CheckPermissionsTool {
    fn def(&self) -> &ToolDef {
        PERMS_DEF.get_or_init(|| ToolDef {
            ...
            description: "Check required permissions for Buzzard CUA on Linux.".into(),
            input_schema: json!({"type":"object","properties":{},"additionalProperties":false}),
            read_only: true,
            destructive: false,
            idempotent: true,
    ...
    async fn invoke(&self, _args: Value) -> ToolResult {
        ...
        let atspi_ok = tokio::task::spawn_blocking(crate::platform::health_report::probe_a11y_bus)
            .await
            .unwrap_or(false);
        ...
        let atspi_status = if atspi_ok {
            ...
            "❌ no session bus (DBUS_SESSION_BUS_ADDRESS unset and none auto-discovered) — \
             AT-SPI trees will be empty; run cua from the Buzzard desktop environment"
                .to_string()
        } else {
        ...
        let status_text = format!(
            ...
            wayland_display
                .as_ref()
                .map(|display| format!("✅ connected ({display})"))
                .unwrap_or_else(|| "❌ WAYLAND_DISPLAY is unavailable".into()),
            atspi_status,
            if x11_ok {
                "✅ connected"
            } else {
        ...
        ToolResult::text(status_text)
            .with_structured(json!({ "x11": x11_ok, "wayland": wayland_display.is_some(), "wayland_enabled": crate::platform::wayland::wayland_enabled(), "atspi": atspi_ok, "dbus_session_bus_address": dbus_address, "xsend_event": x11_ok }))
    }
}
...
impl Tool for GetConfigTool {
    fn def(&self) -> &ToolDef {
        GCFG_DEF.get_or_init(|| ToolDef {
            ...
            description: "Return current Buzzard CUA configuration.".into(),
            input_schema: json!({"type":"object","properties":{},"additionalProperties":false}),
            read_only: true,
            destructive: false,
            idempotent: true,
    ...
    async fn invoke(&self, _args: Value) -> ToolResult {
        let cfg = self.state.config.read().unwrap();
        ToolResult::text("Buzzard CUA configuration").with_structured(json!({
            "version": env!("CARGO_PKG_VERSION"),
            "source_sha": option_env!("BUZZARD_CUA_SOURCE_SHA"),
            "platform": "linux",
...
impl Tool for SetConfigTool {
    fn def(&self) -> &ToolDef {
        SCFG_DEF.get_or_init(|| ToolDef {
            ...
            description: "Update Buzzard CUA configuration. capture_mode / \
                max_image_dimension take effect immediately and persist for future calls.\n\n\
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if let Err(error) = write_config_file(&cfg) {
            return ToolResult::error(format!("failed to persist Buzzard CUA config: {error}"));
        }
...
impl Tool for GetAccessibilityTreeTool {
    fn def(&self) -> &ToolDef {
        GAX_DEF.get_or_init(|| ToolDef {
            ...
            description: "Return the complete in-guest desktop accessibility snapshot: running \
                processes, compositor/AT-SPI windows, and each registered application's AT-SPI \
                tree. Every actionable element includes the pid, window_id, element_index, and \
                opaque element_token needed by the action tools. Each application also carries \
                its snapshot_id. Layer-shell desktop controls are included even when they are \
                not ordinary toplevel windows.\n\n\
                Optional max_elements/max_depth bounds apply independently to each application."
                .into(),
            input_schema: json!({"type":"object","properties":{
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let max_depth = args
            ...
            .and_then(Value::as_u64)
            .map(|value| value.max(1) as usize);
        let (procs, windows, mut applications) = tokio::task::spawn_blocking(move || {
            let procs = crate::platform::proc_fs::list_processes();
            let windows = crate::platform::wayland::list_windows_dispatch(None);
            ...
            for window in &windows {
                ...
                let tree = crate::platform::atspi::walk_tree_bounded(
                    pid,
                    window.xid,
                    None,
                    max_elements,
                ...
                let bounds_by_index: std::collections::HashMap<usize, (i32, i32, u32, u32)> = tree
                    .bounds
                    .iter()
                ...
                let elements = tree
                    ...
                    .filter_map(|node| {
                        ...
                        if let Some(selected) = node.selected {
                            element["selected"] = json!(selected);
                        }
                        if let Some(parent) = node.parent_element_index {
                            element["parent_index"] = json!(parent);
                        }
                        if let Some((x, y, width, height)) = bounds_by_index.get(&index).copied() {
                            element["frame"] = json!({"x":x, "y":y, "w":width, "h":height});
                        }
                        Some(element)
                    })
                    .collect::<Vec<_>>();
                applications.push(json!({
                    ...
                    "tree_markdown": tree.tree_markdown,
                    "element_count": elements.len(),
                    "elements": elements,
                }));
            }
            (procs, windows, applications)
        })
        .await
        .unwrap_or_default();

        // Token registration must happen on the dispatch task. Tokio task
        // locals intentionally do not flow into spawn_blocking workers; minting
        // there would put snapshots in the fallback "legacy" runtime and make
        // the very next action reject its fresh token as generation-mismatched.
        for application in &mut applications {
            let Some(pid) = application.get("pid").and_then(Value::as_i64) else {
                continue;
            };
            let Some(window_id) = application.get("window_id").and_then(Value::as_u64) else {
                continue;
            };
            let element_count = application
                .get("element_count")
                .and_then(Value::as_u64)
                .unwrap_or(0) as usize;
            let snapshot_id = crate::core::element_token::global().register_snapshot(
                pid as i32,
                window_id as u32,
                element_count,
            );
            let snapshot_handle = crate::core::element_token::token_for(snapshot_id, 0)
                .trim_end_matches(":0")
                .to_string();
            application["snapshot_id"] = json!(snapshot_handle.clone());
            if let Some(elements) = application
                .get_mut("elements")
                .and_then(Value::as_array_mut)
            {
                let element_keys = elements
                    .iter()
                    .filter_map(|element| element.get("_element_key").and_then(Value::as_u64))
                    .collect::<Vec<_>>();
                let synthetic_nodes = element_keys
                    .iter()
                    .enumerate()
                    .map(|(index, key)| crate::platform::atspi::AtspiNode {
                ...
                for element in elements {
                    ...
                    element
                        .as_object_mut()
                        .expect("AT-SPI element is an object")
                        .remove("_element_key");
                    element["element_token"] = json!(crate::core::element_token::token_for(
                        snapshot_id,
                        index as usize
                    ));
                }
        ...
        let mut lines = vec![format!(
            "{} running process(es), {} window(s), {} AT-SPI application tree(s)",
            procs.len(),
            windows.len(),
            applications.len()
        )];
        ...
        for application in &applications {
            lines.push(format!(
                "- AT-SPI pid={} \"{}\": {} actionable element(s)",
                application["pid"],
                application["title"].as_str().unwrap_or("(untitled)"),
                application["element_count"]
            ));
        }
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        match result {
            Ok(Ok(crop)) => {
                ...
                use crate::core::protocol::Content;
                ToolResult {
                    content: vec![
...
impl Tool for TypeTextCharsTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        // crate::core::text_sanitize for rationale.
        let text =
            crate::core::text_sanitize::strip_trailing_agent_protocol_tags(&text_raw).into_owned();
        ...
        let result = tokio::task::spawn_blocking(move || {
            if crate::platform::wayland::wayland_input_enabled() {
                return crate::platform::wayland::type_text_with_delay(
                    &keyboard_admission,
                    xid,
                    &text,
                    delay_ms,
            ...
            crate::platform::input::send_type_text_with_delay(xid, &text, delay_ms)
        })
        .await;
        match result {
            Ok(Ok(())) => ToolResult::text(format!(
        ...
        if rc == 0 {
            ...
            ToolResult::error(format!(
                "kill_app: kill(pid={pid_i}, SIGKILL) failed: {err}. \
                 The process may not exist, or CUA lacks permission to signal it."
            ))
        }
    }
}
...
impl Tool for InvokeMenuTool {
    fn def(&self) -> &ToolDef {
        INVOKE_MENU_DEF.get_or_init(|| {
            let contract =
                crate::contract::tool_contract("invoke_menu").expect("invoke_menu contract");
            ToolDef {
                name: contract.name,
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if !crate::platform::wayland::list_windows_dispatch(Some(pid))
            .iter()
        ...
        let activation = if crate::platform::wayland::is_wayland() {
            let result = tokio::task::spawn_blocking(move || {
                crate::platform::wayland::activate_window_for_input_target(window_id, Some(pid))
            })
            .await;
            match result {
                Ok(Ok(())) => None,
            ...
            match tokio::task::spawn_blocking(move || {
                crate::platform::input::x11_activate_window_persistent(window_id)
            })
            .await
            {
        ...
        let outcome = tokio::task::spawn_blocking(move || {
            let result = crate::platform::atspi::native::invoke_menu_path(pid, &path);
            if let Some(Some(prior_window)) = activation {
                let _ = crate::platform::input::x11_activate_window_persistent(prior_window);
            }
            result
        })
        .await;
        ...
        match outcome {
            ...
            .with_structured(json!({
                "verified": false,
                "route": "atspi",
                "evidence": "Every menu hop resolved uniquely and AT-SPI accepted the final action"
            })),
...
struct WindowControlTool {
    kind: WindowControlKind,
}

static CLOSE_WINDOW_DEF: std::sync::OnceLock<ToolDef> = std::sync::OnceLock::new();
static MINIMIZE_WINDOW_DEF: std::sync::OnceLock<ToolDef> = std::sync::OnceLock::new();
static MAXIMIZE_WINDOW_DEF: std::sync::OnceLock<ToolDef> = std::sync::OnceLock::new();
static RESTORE_WINDOW_DEF: std::sync::OnceLock<ToolDef> = std::sync::OnceLock::new();

fn window_control_def(kind: WindowControlKind) -> ToolDef {
    let contract = crate::contract::tool_contract(kind.name()).expect("window control contract");
    ToolDef {
        name: contract.name,
        description: contract.description,
        input_schema: contract.input_schema,
...
fn normalized_wayland_control_state(
    state: crate::platform::wayland::WindowControlState,
) -> (bool, bool, bool, bool) {
    (
        state.present,
        state.minimized,
...
fn normalized_x11_control_state(
    state: crate::platform::x11::WindowControlState,
) -> (bool, bool, bool, bool) {
    (state.present, state.minimized, state.maximized, false)
}

fn window_control_confirmed(kind: WindowControlKind, state: (bool, bool, bool, bool)) -> bool {
    let (present, minimized, maximized, fullscreen) = state;
    match kind {
        WindowControlKind::Close => !present,
        WindowControlKind::Minimize => present && minimized,
        WindowControlKind::Maximize => present && maximized,
        WindowControlKind::Restore => present && !minimized && !maximized && !fullscreen,
    }
}

#[async_trait]
impl Tool for WindowControlTool {
    fn def(&self) -> &ToolDef {
        match self.kind {
            WindowControlKind::Close => {
                CLOSE_WINDOW_DEF.get_or_init(|| window_control_def(self.kind))
            }
            WindowControlKind::Minimize => {
                MINIMIZE_WINDOW_DEF.get_or_init(|| window_control_def(self.kind))
            }
            WindowControlKind::Maximize => {
                MAXIMIZE_WINDOW_DEF.get_or_init(|| window_control_def(self.kind))
            }
            WindowControlKind::Restore => {
                RESTORE_WINDOW_DEF.get_or_init(|| window_control_def(self.kind))
            }
        }
    }
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        let parsed = match self.kind {
            WindowControlKind::Close => crate::core::tool_args::parse_typed_input::<
                crate::contract::CloseWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
            WindowControlKind::Minimize => crate::core::tool_args::parse_typed_input::<
                crate::contract::MinimizeWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
            WindowControlKind::Maximize => crate::core::tool_args::parse_typed_input::<
                crate::contract::MaximizeWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
            WindowControlKind::Restore => crate::core::tool_args::parse_typed_input::<
                crate::contract::RestoreWindowInput,
            >(self.kind.name(), args)
            .map(|input| (input.pid, input.window_id)),
        };
        let (pid, window_id) = match parsed {
            Ok(target) => target,
            Err(result) => return result,
        };
        let kind = self.kind;
        let result = if crate::platform::wayland::is_wayland() {
            let target_exists = crate::platform::wayland::list_windows_dispatch(Some(pid))
                .into_iter()
                .any(|window| window.xid == window_id && window.pid == Some(pid));
            if !target_exists {
                return ToolResult::error(format!(
                    "{}: window_id {window_id} is not a current Wayland toplevel owned by pid {pid}",
                    kind.name()
                ));
            }
            tokio::task::spawn_blocking(move || {
                let action = match kind {
                    WindowControlKind::Close => {
                        crate::platform::wayland::WindowControlAction::Close
                    }
                    WindowControlKind::Minimize => {
                        crate::platform::wayland::WindowControlAction::Minimize
                    }
                    WindowControlKind::Maximize => {
                        crate::platform::wayland::WindowControlAction::Maximize
                    }
                    WindowControlKind::Restore => {
                        crate::platform::wayland::WindowControlAction::Restore
                    }
                };
                crate::platform::wayland::control_window(window_id, pid, action).map(
                    |(before, after)| {
                        (
            ...
            tokio::task::spawn_blocking(move || {
                let action = match kind {
                    WindowControlKind::Close => crate::platform::x11::WindowControlAction::Close,
                    WindowControlKind::Minimize => {
                        crate::platform::x11::WindowControlAction::Minimize
                    }
                    WindowControlKind::Maximize => {
                        crate::platform::x11::WindowControlAction::Maximize
                    }
                    WindowControlKind::Restore => {
                        crate::platform::x11::WindowControlAction::Restore
                    }
                };
                crate::platform::x11::control_window(window_id, pid, action).map(
                    |(before, after)| {
                        (
                            normalized_x11_control_state(before),
                            normalized_x11_control_state(after),
                            "xwayland",
                        )
                    },
                )
        ...
        let (before, after, route) = match result {
            Ok(Ok(outcome)) => outcome,
            Ok(Err(error)) => {
                return ToolResult::error(format!("{}: {error}", kind.name()));
            }
            Err(error) => {
                return ToolResult::error(format!(
        ...
        let confirmed = window_control_confirmed(kind, after);
        let changed = before != after;
        ToolResult::text(if confirmed {
            format!("{} completed and was verified.", kind.name())
        } else {
            format!(
                "{} was requested, but the resulting window state did not match.",
                kind.name()
            )
        })
        .with_structured(json!({
...
impl Tool for SetWindowFrameTool {
    fn def(&self) -> &ToolDef {
        SET_WINDOW_FRAME_DEF.get_or_init(|| {
            let contract = crate::contract::tool_contract("set_window_frame")
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        use crate::contract::SetWindowFrameInput;

        let input: SetWindowFrameInput =
            match crate::core::tool_args::parse_typed_input("set_window_frame", args) {
                Ok(input) => input,
                Err(result) => return result,
            };
        ...
        if crate::platform::wayland::is_wayland() {
            let outcome = tokio::task::spawn_blocking(move || {
                crate::platform::wayland::set_window_frame(window_id, pid, x, y, width, height)
                    .map_err(|error| error.to_string())
            })
            .await;
            let (observed, confirmed, changed) = match outcome {
                Ok(Ok(outcome)) => outcome,
                Ok(Err(error)) => {
                    return ToolResult::error(format!("set_window_frame: {error}"));
                }
                Err(error) => {
                    return ToolResult::error(format!(
            ...
            return ToolResult::text(if confirmed {
                "Set and verified the requested Wayland window frame."
            } else if observed_frame.is_none() {
                "Sway accepted the mutation, but its resulting frame could not be read back."
            } else {
                "The Wayland window frame did not settle at the requested geometry."
            })
            .with_structured(json!({
                "verified": confirmed,
                "changed": changed,
        ...
        let outcome = tokio::task::spawn_blocking(move || {
            let before = crate::platform::x11::list_windows(Some(pid))
                .into_iter()
            ...
            let (observed, confirmed, mutation_error) =
                crate::platform::x11::set_window_frame(window_id, pid, x, y, width, height)
                    .map_err(|error| error.to_string())?;
            let observed_frame =
                observed.map(|window| (window.x, window.y, window.width, window.height));
...
impl Tool for BringToFrontTool {
    fn def(&self) -> &ToolDef {
        BTF_DEF.get_or_init(|| ToolDef {
            ...
            description:
                "Move the exact window into this caller's cuaN workspace and focus it \
                 for seatN. The human Desktop and other numbered seats are unaffected."
                    .into(),
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if crate::platform::wayland::is_wayland() {
            let window_id = match args.opt_u64("window_id") {
                ...
                None => match crate::platform::wayland::list_windows_dispatch(Some(pid)).first() {
            ...
            return match result {
                Ok(Ok(())) => {
                    ToolResult::text(format!("Focused Wayland window {window_id} in CUA workspace {cua_index}."))
                        .with_structured(serde_json::json!({
                            "window_id": window_id,
                            "platform": "linux",
                            "workspace_index": cua_index,
                            "workspace": crate::platform::wayland::sway_ipc::cua_workspace_name(cua_index).ok(),
                        }))
                }
                Ok(Err(error)) => ToolResult::error(error.to_string()),
                Err(error) => ToolResult::error(format!("Task error: {error}")),
        ...
        let r = tokio::task::spawn_blocking(move || {
            crate::platform::input::x11_activate_window_persistent(xid)
        })
        .await;
        match r {
...
pub fn build_registry() -> ToolRegistry {
    // Shutdown cancellation exists before the first keyboard tool is admitted.
    // The Wayland keyboard connection remains lazy until actual key input.
    ...
    r.register(Box::new(crate::core::expectation::VerifyStateTool::new(
        std::sync::Arc::new(crate::core::expectation::ToolObservationProvider::new(
    ...
    crate::core::clipboard::register_clipboard_tools(
        &mut r,
        Arc::new(crate::platform::clipboard::LinuxClipboard::new()),
    );
    ...
    r.register(Box::new(crate::core::health_report::HealthReportTool::new(
        std::sync::Arc::new(crate::platform::health_report::LinuxHealthProvider),
    )));
...
mod click_button_schema_tests {
    use super::{
        chromium_background_must_refuse, click_can_use_atspi_action, maps_indicate_gtk, ClickTool,
    };
    ...
    fn chromium_background_requires_focus_free_inject_mode() {
        assert!(chromium_background_must_refuse(false, true));
        assert!(!chromium_background_must_refuse(true, true));
        assert!(!chromium_background_must_refuse(false, false));
    }

    #[test]
    fn only_one_unmodified_left_click_can_be_represented_by_atspi_activation() {
        assert!(click_can_use_atspi_action(true, 1, 1));
        assert!(!click_can_use_atspi_action(true, 1, 2));
        assert!(!click_can_use_atspi_action(true, 2, 1));
        assert!(!click_can_use_atspi_action(true, 3, 1));
        assert!(!click_can_use_atspi_action(false, 1, 1));
    }
...
mod pid_window_target_tests {
    use super::*;
    use crate::core::window_target::{resolve_pid_window_target, PidWindowTargetResolution};

    fn window(xid: u64, pid: u32) -> crate::platform::x11::WindowInfo {
        crate::platform::x11::WindowInfo {
            xid,
            pid: Some(pid),
