...
pub fn list_installed_apps() -> Vec<InstalledApp> {
    list_installed_apps_in_dirs(&xdg_application_dirs())
}

fn list_installed_apps_in_dirs(roots: &[PathBuf]) -> Vec<InstalledApp> {
    ...
    for root in roots {
        ...
        for entry in entries.flatten() {
            ...
            // XDG data-directory precedence applies to the desktop-file ID,
            // including a Hidden/NoDisplay tombstone. A higher-priority
            // override that suppresses an entry must prevent the lower
            // system copy from making the helper reappear.
            ...
            seen.insert(app.bundle_id.clone(), app);
        }
    }
...
";
    ...
    fn hidden_high_priority_entry_suppresses_visible_system_copy() {
        ...
        std::fs::write(
            high.join("helper.desktop"),
            "[Desktop Entry]\nType=Application\nName=Helper\nHidden=true\n",
        )
        .unwrap();
        std::fs::write(
            low.join("helper.desktop"),
            "[Desktop Entry]\nType=Application\nName=Helper\nExec=helper\n",
        )
        .unwrap();

        assert!(list_installed_apps_in_dirs(&[high, low]).is_empty());
