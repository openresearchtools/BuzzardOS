...
impl ToolRegistry {
    ...
    /// Invoke one local tool without a transport, daemon, observer, policy
    /// adapter, or automatic recording layer. Buzzard CUA runs inside its
    /// dedicated Sway guest and performs admission at the CLI boundary.
    pub async fn invoke_direct(&self, name: &str, args: Value) -> ToolResult {
        let resolved_name = match name {
            "type_text_chars" => "type_text",
            other => other,
        };
        match self.tools.get(resolved_name) {
