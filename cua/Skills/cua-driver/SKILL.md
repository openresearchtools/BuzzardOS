  ...
  openclaw:
    ...
    envVars:
      - name: CUA_DRIVER_PATH
        required: false
        description: Optional path to the source-built Linux cua-driver binary.
...
Orchestrates Linux guest app automation via `cua-driver`. Whenever
...
## Linux reading — read this first

This file defines the snapshot invariant, CLI/MCP choice, canonical loop,
pixel contract, and failure handling. Read `LINUX.md` for Sway IPC, AT-SPI,
native Wayland, Xwayland, capture, and input details.
...
When in doubt, run `cua-driver doctor`; every reported route must remain inside
the guest session.
...
2. **Background accessibility action.** Use a fresh AT-SPI target.
...
2. For a multi-selection, use Linux `ctrl`. A refused background attempt is an
   escalation signal, not a failed action to trust or repeat. Re-snapshot
   before the next operation. Continue only after confirming every intended item is selected;
   also confirm that the prior selection was preserved.
...
`auto` session, **the guest's frontmost app MUST NOT change.** Do not bypass
the driver with shell commands that raise a guest window or inject host input.
...
- `cua-driver serve` — start the persistent guest service when short-lived
  clients must share runtime state; `cua-driver mcp --socket <endpoint>`
  selects it explicitly. One-shot CLI tool calls use the same guest service.
...
Canonical Linux guest workflow:

```bash
cua-driver serve
cua-driver launch_app '{"name":"thunar"}'
...
**Visibility caveat (AT-SPI runs).** On a pure accessibility-action run
...
first to put the cursor on-screen; subsequent AT-SPI actions then glide the
full path normally.

Requires the guest compositor/session UI event loop. Service and private-worker
runtimes provide one. One-shot CLI adapters do not own an overlay themselves.
...
- **element accessibility action** — pass `element_token` (preferred), or the exact
  `element_index` + `snapshot_id` pair from the same response.
  Dispatches through the **AT-SPI accessibility rung**. Backgroundable,
...
`element`↔`element_index`, `px`↔pixel `x,y`. We retired the word "vision"
...
`element_token` — it targets the field, no pre-click. On some Electron
surfaces the accessibility layer can echo the write without rendering it,
...
button, a collapsed field), use its AT-SPI action to open it first
...
**`set_value` stays accessibility-only by design** — use it when the intent is to
replace a control's whole value: dropdowns, checkboxes, sliders, steppers,
and native text fields such as a file manager's inline rename editor. Use
...
**`foreground`, not `pixel`**. Xwayland may support additional background
routes. See `LINUX.md`.
...
# Route 2 — element AT-SPI action, backgrounded
...
The two ideas to hold onto: (1) an accessibility tree can be incomplete on
canvas, web, or virtualized surfaces, so an unchanged-or-bogus tree plus
...
| state                      | `get_window_state`                                                                             | element-index click (AT-SPI) | `press_key` commit                                    | pixel click                    |
| -------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------- | ----------------------------------------------------- | ------------------------------ |
| frontmost                  | ✅                                                                                             | ✅                           | ✅                                                    | ✅                             |
...
- `tree_markdown` — every actionable element tagged `[N]`; the structured row
  with the same `element_index` carries its opaque `element_token`. The tree can be very large; when it exceeds token limits the MCP
...
- `effect` / `escalation` / `degraded` — the verify-then-escalate
  ...
  tree came back empty (non-accessible surface), so you act by **`px`** off the
...
```bash
# write to file — stdout stays readable (AT-SPI tree / summary only, no base64)
...
| Double-click / open              | `double_click({pid, element_token})`                                                                            | Default action when the element advertises one (openable file-manager items or rows), else stamped pixel double-click at the element's center                                                                         |
...
| Set whole non-text control value | `set_value({pid, element_token, value})`                                                                         | **AT-SPI-only by design** — dropdown, checkbox, slider, or stepper. For text use `type_text`; to pixel-manipulate a control use `click`/`drag`                                                                         |
...
## Linux parameter contract

The capture, dispatch, and addressing params — `session`, `delivery_mode`,
`capture_mode` (deprecated and ignored), `scope`, `modifier`, `button`,
`element_index`, `snapshot_id`, and `element_token` — form the stable Linux
schema. They compose from canonical fragments in
`cua-driver-core::tool_schema`; `schema_consistency_test` checks every live
tool registration against those fragments.

- **`session` is accepted on every action and cursor tool.** It selects the
  guest session's cursor, capture scope, and evidence state.
- **`delivery_mode` (`"background"` default / `"foreground"`) is on the
  whole input family.** Foreground is a reaction, never a prediction. Try the
  background route first and escalate only after structured refusal or an
  observed no-op. Wayland and Xwayland limits are documented in `LINUX.md`.

**Required-set contract.** `click` requires nothing (`required:[]`), `scroll`
requires `["direction"]`, and `zoom` requires
`["window_id","x1","y1","x2","y2"]`. `pid` is conditionally required
unless a windowless desktop-scope call is used. Unknown non-Linux platform
parameters are not part of this fork's supported contract.
...
- `click({pid, x, y, modifier: ["ctrl"]})` — modifier click.
  Linux accepts the supported subset of `shift/alt/ctrl`.
- `right_click({pid, x, y})` — also takes `modifier`.

The pixel path animates the agent cursor overlay and uses the Linux route
documented in `LINUX.md`. If the pid has no on-screen window the call errors
with `pid X has no on-screen window`; a visible guest window is required to
...
Use the native `get_window_state` and AT-SPI/pixel action ladder for browser chrome,
permission prompts, downloads, file pickers, Firefox, Tauri, and any
...
| `AT-SPI action … was rejected`                                                     | Element doesn't support or accept the requested action                                                                                                                            | Try another advertised action or fall through to a pixel click on the element's center                                                                                                                               |
...
Linux AT-SPI, Sway, Wayland, Xwayland, and input-route errors are documented in
`LINUX.md`.
...
Linux application and compositor edge cases live in `LINUX.md`.
