...
impl Shell {
    ...
    fn click_surface(&mut self, surface: &wl_surface::WlSurface, x: f64, y: f64) {
        let target = if surface == self.panel.wl_surface() {
    ...
    fn draw_panel(&mut self) -> Result<()> {
        ...
        for target in panel_targets(logical_width, &windows, self.task_page) {
            let (color, label) = match target.action {
                ShellAction::ToggleApplications => ([64, 104, 139, 255], "Applications".to_owned()),
                ShellAction::OpenFiles => ([60, 66, 74, 255], "Files".to_owned()),
                ShellAction::OpenShared => ([60, 66, 74, 255], "Share".to_owned()),
                ShellAction::ActivateWindow(id) => {
                ...
                ShellAction::TaskbarPrevious => ([60, 66, 74, 255], "‹".to_owned()),
                ShellAction::TaskbarNext => ([60, 66, 74, 255], "›".to_owned()),
                _ => continue,
            };
            fill_rect(
                canvas,
    ...
    fn draw_menu(&mut self) -> Result<()> {
        ...
        if self.menu_open && self.menu_kind == MenuKind::Applications {
            ...
            for target in menu_targets(logical_height, &self.applications, self.menu_scroll) {
    ...
    fn accessibility_tree(&self) -> (TreeUpdate, BTreeMap<NodeId, AccessibleTarget>) {
        ...
        if let Some(target) = panel_targets(self.panel_size.0, &windows, self.task_page)
            .into_iter()
        ...
        for (index, application) in self.applications.iter().enumerate() {
            ...
            if applications_menu_open {
                node.set_bounds(a11y_rect(
                    Rect {
                        x: 8,
                        y: 54 + 26 + relative_row.saturating_mul(MENU_ROW_HEIGHT),
                        width: MENU_WIDTH - 16,
...
fn draw_menu_icon(canvas: &mut [u8], width: u32, height: u32, rect: Rect, action: &ShellAction) {
    let color = match action {
        ShellAction::OpenFiles | ShellAction::OpenShared => [233, 179, 56, 255],
        ShellAction::ShutdownMachine => [177, 78, 78, 255],
        ShellAction::LaunchApplication(_) => [92, 139, 103, 255],
        _ => [105, 113, 121, 255],
    };
