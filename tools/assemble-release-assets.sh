...
for command_name in cmp find git install jq python3 realpath sha256sum stat tar xz; do
...
jq -n \
    ...
    --arg archive "default-rootfs.oci.tar.zst" \
    ...
    '{schema:1,kind:"buzzardos-portable",platform:{os:"linux",architecture:"x86_64"},source_commit:$source_commit,rootfs:{archive:$archive,sha256:$archive_sha256},icon:{source:"low-glide-source.png",sha256:$icon_sha256}}' \
...
(
    cd "$output_dir"
    sha256sum "$(basename -- "$final_archive")" \
        >"$(basename -- "$final_archive").sha256"
    sha256sum --check --strict "$(basename -- "$final_archive").sha256"
)
