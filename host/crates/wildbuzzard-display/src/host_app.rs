...
impl NativeWindow {
    ...
    fn install_input_handlers(self: &Rc<Self>) {
        ...
        buttons.connect_event(move |_, event| {
            ...
            if pressed {
                ...
                {
                    eprintln!(
                        "wildbuzzard-display: recording guest pointer click failed: {error:#}"
                    );
                }
    ...
    fn poll(&self) {
        ...
        if self.last_runtime_check.get().elapsed() >= RUNTIME_POLL_INTERVAL {
                ...
                && let Err(error) = self.save_offload_verification()
            {
                ...
                eprintln!(
                    "wildbuzzard-display: saving GTK offload verification diagnostics: {error:#}"
                );
            }
                ...
                && self.last_continuity_save.get().elapsed() >= CONTINUITY_SAVE_INTERVAL
            {
                ...
                if let Err(error) = self.save_continuity() {
                    ...
                    eprintln!("wildbuzzard-display: saving monitor continuity: {error:#}");
                }
            }
        }
            ...
            CursorStorage::Dmabuf {
                ...
                let texture = match unsafe {
                    ...
                    Err(error) => {
                        ...
                        eprintln!(
                            "wildbuzzard-display: GTK rejected guest cursor dmabuf; \
        ...
        if let Some(frame) = self.pending_frame.borrow_mut().take() {
            ...
            let offloaded = {
                ...
                match verifier.poll() {
                    ...
                    Err(error) => {
                        eprintln!(
                            "wildbuzzard-display: reading GTK offload verification log: {error}"
                        );
                        false
                    }
                }
    ...
    fn reset_offload_verification(
        ...
        if let Err(error) = self.offload_verifier.borrow_mut().reset(kind, expected) {
            eprintln!(
                "wildbuzzard-display: resetting GTK offload verification generation: {error}"
            );
        }
        self.offload_verification_dirty.set(true);
        self.presentation_dirty.set(true);
    ...
    fn observe_monitor_continuity(&self, source: &'static str) {
        ...
        if violation {
            eprintln!(
                "wildbuzzard-display: monitor continuity violation: source={source}, \
