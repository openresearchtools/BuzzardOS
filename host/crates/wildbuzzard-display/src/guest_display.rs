...
impl GuestState {
            ...
            match cursor_image_from_buffer(cursor_id, &buffer, self.cursor_hotspot) {
                Ok(image) => {
                    ...
                    if let Err(error) = self.events.send(GatewayEvent::GuestCursor(image)) {
                        eprintln!("wildbuzzard-display: forwarding guest cursor: {error:#}");
                        if leased {
                ...
                Err(error) => {
                    ...
                    eprintln!(
                        "wildbuzzard-display: using host default cursor after guest cursor \
    ...
    fn release_cursor_commit(
        ...
        if let Some(point) = release_point
            && let Err(error) = point.timeline.signal(point.point)
        {
            eprintln!("wildbuzzard-display: signaling guest cursor release: {error:#}");
        }
        if !explicit_sync {
            buffer.release();
        }
...
fn create_dmabuf_format_table(formats: &[DmabufFormat]) -> Result<(File, u32)> {
    ...
    let name = b"wildbuzzard-dmabuf-formats\0";
...
impl GlobalDispatch<wl_seat::WlSeat, ()> for GuestState {
    fn bind(
        ...
        if seat.version() >= 2 {
            seat.name("wildbuzzard-seat".into());
        }
    }
}
